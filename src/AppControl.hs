...
staticRoutes = choice 
     ...
     , hGetAllowHttp $ getContext >>= (redirectKontraResponse . LinkHome . getLocale)
     ...
     , dir "adminonly" $ dir "sysdump" $ hGet $ toK0 $ sysdump
     ...
     , dir "adminonly" $ dir "log" $ hGetWrap (onlySuperUser . https) $ toK1 $ serveLogDirectory


     , dir "dave" $ dir "document" $ hGet $ toK1 $ daveDocument
     , dir "dave" $ dir "user"     $ hGet $ toK1 $ daveUser
     , dir "dave" $ dir "company"  $ hGet $ toK1 $ daveCompany
