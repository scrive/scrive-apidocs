module EID.CGI.GRP.Tables where
...
mergeCgiGrpTransaction = SQLFunction $ smconcat [
  ...
  , "  transaction_id_ TEXT, order_ref_ TEXT) RETURNS VOID AS $$"
  , "BEGIN"
  , "  LOOP"
  ...
  , "        (signatory_link_id, session_id, transaction_id, order_ref)"
  , "      VALUES"
  , "        (signatory_link_id_, session_id_, transaction_id_, order_ref_);"
  , "      RETURN;"
