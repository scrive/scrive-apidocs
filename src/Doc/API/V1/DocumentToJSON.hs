...
jsonDate mdate = toJSValue $ formatMinutesTimeISO <$> mdate
...
docFieldsListForJSON userid doc = do
    ...
    J.value "time" $ formatMinutesTimeISO (documentmtime doc)
    J.value "ctime" $ formatMinutesTimeISO (documentctime doc)
    J.value "timeouttime" $ formatMinutesTimeISO <$> documenttimeouttime doc
    J.value "template" $ isTemplate doc
...
signatoryFieldsListForJSON doc sl = do
    ...
    J.value "time" $ fromMaybe "" $ formatMinutesTimeISO <$> (sign `mplus` reject `mplus` seen `mplus` open)
...
signatoryForListCSV _agr doc msl = [
    ...
    where
        csvTime = formatMinutesTimeSimple
