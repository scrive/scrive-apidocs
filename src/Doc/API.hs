...
documentNew = api $ do
  ...
  pdfcontent <- case mformat of
    ...
    Just format -> do
      ...
      case eres of
        Left (LiveDocxIOError _) -> throwError ServerError
        Left (LiveDocxSoapError _)-> throwError ServerError
  ...
  content <- apiGuardL' BadInput $ liftIO $ preCheckPDF (ctxgscmd ctx) (concatChunks pdfcontent)
