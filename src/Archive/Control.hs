...
jsonDocumentsList = withUserGet $ do
  ...
  allDocs <- case (doctype) of
    ...
    "Pad" -> filter padDocument <$> (runDBQuery $ GetDocumentsByAuthor  uid)
    _ -> do
  ...
  docsJSONs <- mapM (fmap JSObject . docForListJSON (timeLocaleForLang lang) cttime user padqueue) $ list docs
