...
handleSignNordea :: DocumentID -> SignatoryLinkID -> MagicHash -> Kontra Response
handleSignNordea docid signid magic = do
  -- document <- queryOrFail $ GetDocumentByDocumentID docid
  -- checkLinkIDAndMagicHash document signid magic
  ctx <- get
...
handleSignPostNordea :: DocumentID -> SignatoryLinkID -> MagicHash -> Kontra KontraLink
handleSignPostNordea docid signid magic = do
  document <- queryOrFail $ GetDocumentByDocumentID docid
  checkLinkIDAndMagicHash document signid magic
                  ...
                  Right (cert, attrs) -> do
                            ...
                            case newdocument of
                              ...
                              Right document -> do 
                                               postDocumentChangeAction document olddocumentstatus (Just signid)
                                               return $ LinkSigned docid signid
handleIssueNordea :: DocumentID -> Kontra Response
handleIssueNordea docid = do
  ctx <- get
...
handleIssuePostNordea :: DocumentID -> Kontra KontraLink
handleIssuePostNordea docid = withUserPost $ do
  document <- queryOrFail $ GetDocumentByDocumentID $ docid
  ctx@Context { ctxmaybeuser = Just user, ctxelegtransactions } <- get
  failIfNotAuthor document user
