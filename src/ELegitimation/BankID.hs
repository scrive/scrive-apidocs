...
handleSignBankID docid signid magic = do
    ...
    case nonceresponse of
        ...
        Right (nonce, transactionid) -> do
            ...
            tbs <- getTBS ctxtemplates document
...
handleSignPostBankID docid signid magic = do
  ...
  when (any BS.isNull fieldvalues) $ do
    addFlashMsgText "Please fill out all fields."
    return $ LinkSign docid signid magic
  ...
  case done of
    ...
    Left (ImplStatus _a _b code msg) -> do
              liftIO $ print $ toJSON [("status", JInt code)
                                      ,("msg", JString msg)]
              addFlashMsgText "E-Legitimation request failed: " ++ msg
              return $ LinkSign docid signid magichash
    -- successful request
    Right (cert, attrs) -> do
      ...
      Just (SignatoryLink { signatorydetails = details }) <- signlinkFromDocByID document signid
      let (contractFirst, contractLast) = splitName $ signatoryname details
          contractNumber = mergeTwo $ signatorynumber details $ fieldvaluebyid "signr" fields

          elegFirst  = fieldvaluebyid "First Name" attrs
          elegLast   = fieldvaluebyid "Last Name"  attrs
          elegNumber = fieldvaluebyid "Number"     attrs
      ...
      case mfinal of
        ...
        Right (finalFirst, finalLast, finalNumber) -> do
          let finalName = finalFirst ++ " " ++ finalLast
          
          newdocument <- update $ SignDocument docid signid ctxtime ctxipnumber (Just signinfo) fields finalName finalNumber
          case newdocument of
            -- signature failed
            Left message -> do
...
{- | Compare signatory information from contract with that from the
     ...
     correct value.
 -}
mergeInfo :: (String, String, String) -> (String, String, String) -> Either String (String, String, String)
...
splitName full = splitName' "" full

splitName' acc [] = ("", acc)
splitName' acc (x:xs) 
    | x == ' '  = (acc, xs)
    | otherwise = splitName' (acc ++ [x]) xs
...
findTransactionByIDOrFail transactions transactionsid = 
    case find ((== transactionid) . transactiontransactionid) transactions of
      Nothing                                                         -> mzero
...
getSigEntries templates doc = 
    map $ (getSigEntry templates) . signatorydetails $ documentsignatorylinks doc
...
getSigEntry templates signatorydetails =
    renderTemplate templates "tbssig" $ do
        ...
        field "lastname"  $ signatorylstname signatorydetails
