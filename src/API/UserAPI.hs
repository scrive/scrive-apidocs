...
sendNewDocument = do
  ...
  _ <- runDBUpdate $ SetDocumentFunctionality (documentid newdoc) AdvancedFunctionality (ctxtime ctx)
  _ <- runDBUpdate $ SetDocumentIdentification (documentid newdoc) [EmailIdentification] (ctxtime ctx)
