...
csvUtilTests _ = testGroup "CSVUtil"
    [ testGroup "parseCSV"
      ...
      , testCase "exported from excel on a mac in dos csv format" $ testCSVFile "mac_dos_csv.csv"
      , testCase "exported from excel on windows in csv format" $ testCSVFile "windows_csv.csv"
      , testCase "exported from excel on windows in mac format" $ testCSVFile "windows_mac.csv"
      , testCase "exported from excel on windows in ms-dos format" $ testCSVFile "windows_ms_dos.csv"
...
getCSV file = do
  contents <- BSL.readFile $ "test/csv/" ++ file
