...
sendDocumentMails mailTo author = do
  forM_ allLangs $ \l ->  do
        ...
        -- DELIVERY MAILS
        checkMail "Deferred invitation"    $  mailDeferredInvitation Nothing (ctxhostpart ctx) doc sl
        checkMail "Undelivered invitation" $  mailUndeliveredInvitation Nothing (ctxhostpart ctx) doc sl
        checkMail "Delivered invitation"   $  mailDeliveredInvitation Nothing (ctxhostpart ctx) doc sl
        --remind mails
