...
createUserAndResetPassword = do
  ...
  Just user <- dbUpdate $ AddUser ("", "") "andrzej@skrivapa.se" (Just pwd) (companyid company) defaultValue Nothing
...
createTestUser = do
    pwd <- createPassword "admin"
    company <- dbUpdate $ CreateCompany
    Just User{userid} <- dbUpdate $ AddUser ("", "") "andrzej@skrivapa.se" (Just pwd) (companyid company) defaultValue Nothing
    return userid
