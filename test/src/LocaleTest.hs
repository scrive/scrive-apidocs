...
testLoggedInLocaleSwitching = do
  ...
  where
    ...
    assertLoggedInAndOnUploadPage uid res ctx = do
      ...
      assertBool "Location is /newdocumentorlatestdraft" $ T.getHeader "location" (rsHeaders res) == Just "/newdocumentorlatestdraft"
...
createTestUser region lang = do
    ...
    Just user <- dbUpdate $ AddUser ("", "") "andrzej@skrivapa.se" (Just pwd) Nothing (mkLocale region lang)
    return user
