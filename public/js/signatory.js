...
function placementToHTML(label, value) {
    ...
    return $("<div class='placedfield' style='position:absolute'>" + "<span class='value'>" + escapeHTML(v) + "</span>" + "</div>");
}
...
function magicUpdate() {
    ...
    $(".placedfield").each(function() {
        var f = $(this);
        if (getSigID(f) == sigid && getFieldID(f) == fieldid) {
            setValue(f, value);
        }
    });
    updateStatus(field);
...
function isStandardField(field) {
    if (isDraggableField(field)) {
        ...
        if (name == "signatoryemail" || name == "signatoryfstname" || name == "signatorysndname" || name == "signatorycompany" || name == "signatorypersonalnumber" || name == "signatorycompanynumber") {
            return true;
        }
    }
    return false;
...
function docstateToHTML() {
    ...
    $(".signViewBodyRight").each(function() {
        var f = $(this);
        if (f.text().indexOf(useremail) > -1) {
            // hacky; TODO add actual divs with classnames
            currentsigdiv = f;
        }
    });
