...
DHTMLgoodies_dragDrop.prototype = {
	...
	addSource : function(sourceId,slideBackAfterDrop,xAxis,yAxis,dragOnlyWithinElId,functionToCallOnDrag)
	{
		...
		if(!obj) alert('The source element with id ' + sourceId + ' does not exists');

		this.addSourceNode(obj,slideBackAfterDrop,xAxis,yAxis,dragOnlyWithinElId,functionToCallOnDrag);
	}
	,
	addSourceNode : function(sourceNode,slideBackAfterDrop,xAxis,yAxis,dragOnlyWithinElId,functionToCallOnDrag)
	{
		...
		if(yAxis!==false)yAxis = true;
                ...
                var arr = [obj,slideBackAfterDrop,xAxis,yAxis,dragOnlyWithinElId,functionToCallOnDrag];
		this.dragDropSourcesArray[this.dragDropSourcesArray.length]  = arr;
		...
		if( this.dragObjCloneArray!=null ) {
			var el = obj.cloneNode(true);
			el.onmousedown = this.__initDragDropElement;
			el.id = 'DHTMLgoodies_dragableElement' + this.dragObjCloneArray.length;
			...
			el.style.top = dhtmlSuiteCommonObj.getTopPos(obj) + 'px';
			el.style.left = dhtmlSuiteCommonObj.getLeftPos(obj) + 'px';

			obj.onmousedown =this.__initDragDropElement;

			this.dragObjCloneArray[this.dragObjCloneArray.length] = el;
                }
	...
	{
		var refToThis = this;
		if( this.dragDropSourcesArray==null ) {
	...
	__stop_dragDropElement : function(e)
	{
		...
		for(var no=0;no<referenceToDragDropObject.dragDropTargetArray.length;no++){
			...
			if(leftPosMouse > leftPosEl && leftPosMouse < (leftPosEl + widthEl) && topPosMouse > topPosEl && topPosMouse < (topPosEl + heightEl)){
				if(referenceToDragDropObject.dragDropTargetArray[no][1])eval(referenceToDragDropObject.dragDropTargetArray[no][1] + '("' + referenceToDragDropObject.dragDropSourcesArray[referenceToDragDropObject.numericIdToBeDragged][0] + '","' + referenceToDragDropObject.dragDropTargetArray[no][0] + '",' + e.clientX + ',' + e.clientY + ')');
				break;
			}			
		}	
