(function($) {
  ...
  $.fn.tweet = function(o){
    ...
    function relative_time(time_value) {
      ...
      var r = '';
      if (delta < 60) {
	r = delta + ' sekunder sen';
      } else if(delta < 120) {
	r = 'en minut sen';
      } else if(delta < (45*60)) {
	r = (parseInt(delta / 60, 10)).toString() + ' minuter sen';
      } else if(delta < (2*60*60)) {
	r = 'en timme sen';
      } else if(delta < (24*60*60)) {
	r = '' + (parseInt(delta / 3600, 10)).toString() + ' timmar sen';
      } else if(delta < (48*60*60)) {
	r = 'en dag sen';
      } else {
	r = (parseInt(delta / 86400, 10)).toString() + ' dagar sen';
      }
      return 'för ' + r;
    }
      ...
      $(widget).bind("load", function(){
        $.getJSON(build_url(), function(data){
          ...
          $.each(tweets, function(i,item){
            ...
            var text = '<span class="tweet_text"><a href="http://'+s.twitter_url+'/'+from_user+'/statuses/'+item.id_str+'" title="view tweet on twitter">' +$([item.text]).makeHeart().capAwesome().capEpic()[0]+ '</a></span>';
