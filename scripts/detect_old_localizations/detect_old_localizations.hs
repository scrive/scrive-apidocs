...
-- maps (and its values),
disjointUnion :: Ord k =>
  (k -> a -> a -> x) -> Map.Map k a -> Map.Map k a -> Either x (Map.Map k a)
disjointUnion f m1 m2 =
  case Map.minViewWithKey intersection of
    Just ((k, v), _) -> Left $ f k v $ m2 Map.! k
    Nothing -> Right $ m1 `Map.union` m2
  where intersection = m1 `Map.intersection` m2

disjointUnions :: Ord k => (k -> a -> a -> x) -> [Map.Map k a]
               -> Either x (Map.Map k a)
...
pruneLocalization (Object m) = Object $ Map.fromList $
                               filter (\(_, v) -> nonEmptyElem v) prunnedElems
  where
    elems = Map.toList m
...
instance Show Localization where
  show = aux 0
    where
      aux _ (Value t) = "\"" ++ t ++ "\""
      aux n (Object m) =
        let foo (k, l) = concat (replicate n " ")
                         ++ k ++ ": " ++ aux (n+4) l
...
intersectLocalizations (Object m1) (Object m2) = Object $ Map.fromList items'
    where
      ...
      aux x@(k, Value _) =
        if k `Map.member` m2 then Just x else Nothing
...
removeLocalizationCallFromLocalization
  (LocalizationCall path finalElem filePath' lineNumber) loc = aux path loc
  where
    myError s = Left $ filePath' ++ ":" ++ show lineNumber
                ++ ": " ++ s ++ " (from call: '" ++ wholePathFormatted ++ "')"
    wholePathFormatted = format $ path ++
      [case finalElem of
          DictAccess x -> x ++ "["
          FunCall x -> x ++ "("
          Attribute x -> x]
  ...
  where
    aux path = do
      ...
      let numberedLines = zip [1..] $ lines contents
          ...
          results = concatMap
            (\(lineNumber, line) -> map (\x -> (x, path, lineNumber))
              $ getAllTextMatches $ line =~ localizationRegex)
            numberedLines
...
main = do
  ...
  templates_files    <- filter (".st" `isSuffixOf`)
  ...
  let results = map (\lc -> removeLocalizationCallFromLocalization
                            lc mainLocalization)
                localizationCalls
