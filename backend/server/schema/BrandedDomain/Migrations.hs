...
addMainDomain = do
        ...
        runQuery_ . sqlInsert "themes" $  do
          ...
          sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ mailLogo
        ...
        runQuery_ . sqlInsert "themes" $  do
          ...
          sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ mainLogo
        ...
        runQuery_ . sqlInsert "themes" $  do
          ...
          sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ loginLogo
          sqlSet "brand_color" $ ("#495259":: String)
          sqlSet "brand_text_color" $ ("#ffffff":: String)
          sqlSet "action_color" $ ("#53b688":: String)
          sqlSet "action_text_color" $ ("#ffffff":: String)
...
changeScriveSignviewLogo =
  ...
  , mgrDo = do
      ...
      runQuery_ . sqlInsert "themes" $ do
        ...
        sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ signviewLogo
        sqlSet "brand_color" $ ("#495259":: String)
        sqlSet "brand_text_color" $ ("#ffffff":: String)
        sqlSet "action_color" $ ("#53b688":: String)
        sqlSet "action_text_color" $ ("#ffffff":: String)
...
changeRestOfScriveLogos :: (MonadDB m, MonadThrow m) => Migration m
changeRestOfScriveLogos =
  ...
  , mgrDo = do
      ...
      runQuery_ . sqlSelect "branded_domains" $ do
        sqlWhere "main_domain"
        sqlResult "mail_theme"
      (mailThemeId::Int64) <- fetchOne runIdentity
      runQuery_ . sqlSelect "branded_domains" $ do
        sqlWhere "main_domain"
        sqlResult "service_theme"
      (serviceThemeId::Int64) <- fetchOne runIdentity
      runQuery_ . sqlSelect "branded_domains" $ do
        sqlWhere "main_domain"
        sqlResult "login_theme"
      (loginThemeId::Int64) <- fetchOne runIdentity
      runQuery_ . sqlUpdate "themes" $  do
        sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ darkTextLogo
        sqlWhereEq "id" mailThemeId
      runQuery_ . sqlUpdate "themes" $  do
        sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ lightTextLogo
        sqlWhereEq "id" serviceThemeId
      runQuery_ . sqlUpdate "themes" $  do
        sqlSet "logo" $ B64.decodeLenient $ BS.fromString $ lightTextLogo
