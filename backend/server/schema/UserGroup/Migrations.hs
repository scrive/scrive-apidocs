...
userGroupSettingsAddDocumentSessionTimeout :: MonadDB m => Migration m
userGroupSettingsAddDocumentSessionTimeout = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 12
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "document_session_timeout"
                                   , colType     = IntegerT
                                   , colNullable = True
                                   , colDefault  = Nothing
                                   }
...
userGroupSettingsRemoveOldComposites :: MonadDB m => Migration m
userGroupSettingsRemoveOldComposites = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 13
  , mgrAction    = StandardMigration $ do
                     runQuery_ $ sqlDropComposite "user_group_settings_c1"
                     runQuery_ $ sqlDropComposite "user_group_settings_c2"
                     runQuery_ $ sqlDropComposite "user_group_settings_c3"
                     runQuery_ $ sqlDropComposite "user_group_settings_c4"
                     runQuery_ $ sqlDropComposite "user_group_settings_c5"
                     runQuery_ $ sqlDropComposite "user_group_settings_c6"
  }

userGroupSettingsAddForceHidePN :: MonadDB m => Migration m
userGroupSettingsAddForceHidePN = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 14
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "force_hide_pn"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
      ...
      runQuery_ $ sqlDropComposite "user_group_settings_c7"
  }

userGroupSettingsAddPostSignViewFlag :: MonadDB m => Migration m
userGroupSettingsAddPostSignViewFlag = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 15
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "has_post_signview"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
      ...
      runQuery_ . sqlUpdate "user_group_settings as ugs" $ do
        sqlSet "has_post_signview" True  -- enable 'postsignview'

        sqlFrom "user_groups as ug, user_group_invoicings as ugi"
        sqlWhere "ug.id = ugs.user_group_id"
        sqlWhere "ugi.user_group_id = ugs.user_group_id"

        sqlWhereIsNULL "ug.parent_group_id"  -- for root user groups
        sqlWhereEq "ugi.payment_plan" (0 :: Int16)  -- on the 'free' plan

      runQuery_ . sqlCreateComposite $ CompositeType
        { ctName    = "user_group_settings_c10"
        , ctColumns =
          [ CompositeColumn { ccName = "ip_address_mask_list", ccType = TextT }
          , CompositeColumn { ccName = "idle_doc_timeout_preparation"
                            , ccType = SmallIntT
...
addAddMetadataSettingToUserGroupSettings :: MonadDB m => Migration m
addAddMetadataSettingToUserGroupSettings = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 17
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "add_metadata_to_pdfs"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
      ...
      runQuery_ $ sqlDropComposite "user_group_settings_c10"
  }


userGroupSettingsAddEidUseForSEViewFlag :: MonadDB m => Migration m
userGroupSettingsAddEidUseForSEViewFlag = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 18
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "use_eid_service_for_se_view"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
...
userGroupSettingsAddAppFrontendFlag :: MonadDB m => Migration m
userGroupSettingsAddAppFrontendFlag = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 19
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "use_app_frontend"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
      ...
      runQuery_ . sqlCreateComposite $ CompositeType
        { ctName    = "user_group_settings_c14"
        , ctColumns =
          [ CompositeColumn { ccName = "ip_address_mask_list", ccType = TextT }
          , CompositeColumn { ccName = "idle_doc_timeout_preparation"
                            , ccType = SmallIntT
...
userGroupSettingsAddSEBankIDOverride :: MonadDB m => Migration m
userGroupSettingsAddSEBankIDOverride = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 20
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "sebankid_signing_override"
                                   , colType     = SmallIntT
                                   , colNullable = True
                                   , colDefault  = Nothing
                                   }
      ...
      runQuery_ $ sqlDropComposite "user_group_settings_c13"
  }

userGroupSettingsAddPadesCredentialsLabel :: MonadDB m => Migration m
userGroupSettingsAddPadesCredentialsLabel = Migration
  { mgrTableName = tblName tableUserGroupSettings
  , mgrFrom      = 21
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableUserGroupSettings)
        [ sqlAddColumn $ tblColumn { colName     = "pades_credentials_label"
                                   , colType     = TextT
                                   , colNullable = True
                                   , colDefault  = Nothing
                                   }
      ...
      runQuery_ $ sqlDropComposite "user_group_settings_c14"
  }
