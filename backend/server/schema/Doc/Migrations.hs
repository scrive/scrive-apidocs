...
addSealingMethodToDocuments :: MonadDB m => Migration m
addSealingMethodToDocuments = Migration
  { mgrTableName = tblName tableDocuments
  , mgrFrom      = 55
  , mgrAction    =
    StandardMigration $ do
...
addSMSInvitationAndConfirmationTextToDocuments :: MonadDB m => Migration m
addSMSInvitationAndConfirmationTextToDocuments = Migration
  { mgrTableName = tblName tableDocuments
  , mgrFrom      = 56
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableDocuments)
        [ sqlAddColumn $ tblColumn { colName     = "sms_invite_text"
                                   , colType     = TextT
                                   , colNullable = True
                                   }
        ]
      runQuery_ $ sqlAlterTable
        (tblName tableDocuments)
        [ sqlAddColumn $ tblColumn { colName     = "sms_confirm_text"
                                   , colType     = TextT
                                   , colNullable = True
                                   }
        ]
...
addAddMetadataOptionToDocuments :: MonadDB m => Migration m
addAddMetadataOptionToDocuments = Migration
  { mgrTableName = tblName tableDocuments
  , mgrFrom      = 57
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableDocuments)
        [ sqlAddColumn $ tblColumn { colName     = "add_metadata_to_pdf"
                                   , colType     = BoolT
                                   , colNullable = False
                                   , colDefault  = Just "false"
                                   }
      ...
      runQuery_ $ sqlAlterTable (tblName tableDocuments)
                                [sqlAlterColumn "add_metadata_to_pdf" "DROP DEFAULT"]
      runQuery_ . sqlDropComposite $ "document_c1"
      runQuery_ . sqlCreateComposite $ CompositeType
        { ctName    = "document_c1"
        , ctColumns =
