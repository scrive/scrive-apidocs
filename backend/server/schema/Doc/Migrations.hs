...
addSearchColumnsToDocument = Migration {
  ...
  , mgrAction = StandardMigration $ do
      ...
      runQuery_ . sqlCreateIndexSequentially tname $ (indexOnColumn "archive_search_terms") { idxWhere = Just ("archive_search_terms IS NULL") }
      runQuery_ . sqlCreateIndexSequentially tname $ (indexOnColumnWithMethod "archive_search_fts" GIN)
  }
...
signatoryLinkFieldsAddRadioGroupValues = Migration {
  ...
  , mgrAction = StandardMigration $ do
      ...
      runQuery_ $ sqlAlterTable "signatory_link_fields"  $ map sqlAddValidCheck
        [ tblCheck
          { chkName = "check_signatory_link_fields_name_fields_are_well_defined"
          , chkCondition = "type = 1 AND name_order IS NOT NULL AND value_bool IS NULL AND value_file_id IS NULL AND value_text IS NOT NULL AND radio_button_group_values IS NULL OR type <> 1"
          }
        , tblCheck
          { chkName = "check_signatory_link_fields_signatures_are_well_defined"
          , chkCondition = "type = 8 AND name_order IS NULL AND value_bool IS NULL AND value_text IS NULL AND radio_button_group_values IS NULL OR type <> 8"
          }
        , tblCheck
          { chkName = "check_signatory_link_fields_checkboxes_are_well_defined"
          , chkCondition = "type = 9 AND name_order IS NULL AND value_bool IS NOT NULL AND value_file_id IS NULL AND value_text IS NULL AND radio_button_group_values IS NULL OR type <> 9"
          }
        , tblCheck
          { chkName = "check_signatory_link_fields_other_text_fields_are_well_defined"
          , chkCondition = "(type = ANY (ARRAY[3, 4, 5, 6, 7, 10])) AND name_order IS NULL AND value_bool IS NULL AND value_file_id IS NULL AND value_text IS NOT NULL AND radio_button_group_values IS NULL OR NOT (type = ANY (ARRAY[3, 4, 5, 6, 7, 10]))"
          }
        , tblCheck
          { chkName = "check_signatory_link_fields_radio_buttons_are_well_defined"
          , chkCondition = "type = 11 AND name_order IS NULL AND value_bool IS NULL AND value_file_id IS NULL AND radio_button_group_values IS NOT NULL OR type <> 11"
          }
        ]
  }
    ...
    , mgrAction = StandardMigration $ do
        ...
        runQuery_ $ sqlAlterTable "signatory_link_fields"
          [ sqlAddValidCheck $ tblCheck
            ...
            , chkCondition = "(type = ANY (ARRAY[6, 10])) AND editable_by_signatory IS NOT NULL OR (type <> ALL (ARRAY[6, 10])) AND editable_by_signatory IS NULL"
            }
          ]
    }
...
addTemplateInfoToDocuments = Migration
  ...
  , mgrAction = StandardMigration $ do
      ...
      runQuery_ . sqlCreateIndexSequentially "documents" $ indexOnColumn "template_id"
  }
