...
createTableAccessControl = Migration
  ...
  , mgrAction    =
    StandardMigration . createTable True $ tblTable
      ...
      , tblColumns     =
        ...
        , tblColumn { colName     = "src_user_group_id"
                    , colType     = BigIntT
                    , colNullable = True
                    }
        , tblColumn { colName = "trg_user_id", colType = BigIntT, colNullable = True }
        , tblColumn { colName     = "trg_user_group_id"
                    , colType     = BigIntT
                    , colNullable = True
                    }
        ]
      , tblPrimaryKey  = pkOnColumn "id"
      , tblIndexes     = [ indexOnColumn "src_user_id"
...
addFolderTargetColumn = Migration
  { mgrTableName = tblName tableAccessControl
  , mgrFrom      = 3
  , mgrAction    =
    StandardMigration $ do
      runQuery_ $ sqlAlterTable
        (tblName tableAccessControl)
        [ sqlAddColumn
...
addFolderRolesChecks = Migration
  { mgrTableName = tblName tableAccessControl
  , mgrFrom      = 5
...
addFolderAdminRoleChecks = Migration
  { mgrTableName = tblName tableAccessControl
  , mgrFrom      = 6
  , mgrAction    = StandardMigration $ do
                     runQuery_
