...
addAuthenticationKindToEIDAuthentications = Migration
  ...
  , mgrAction    = StandardMigration $ do
                     ...
                     runQuery_ $ sqlAlterTable
                       tname
                       [ sqlDropPK tname
                       , sqlAddColumn tblColumn { colName     = "auth_kind"
                                                , colType     = SmallIntT
                                                , colNullable = False
                                                , colDefault  = Just "1"
                                                }
                     ...
                     runQuery_ $ sqlAlterTable
                       ...
                       , sqlAddPK tname . fromJust $ pkOnColumns
                         ["signatory_link_id", "auth_kind"]
                       ]
  }
...
addProviderCustomerIDToEIDAuthentications :: MonadDB m => Migration m
addProviderCustomerIDToEIDAuthentications = Migration
  { mgrTableName = tblName tableEIDAuthentications
  , mgrFrom      = 7
  , mgrAction    =
    StandardMigration $ do
