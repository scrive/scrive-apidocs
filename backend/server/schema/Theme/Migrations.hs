...
, mgrAction = StandardMigration $ runQuery_ $ do
    sqlAlterTable (tblName tableThemeOwnership)  [
      ...
      , sqlAddValidFK (tblName tableThemeOwnership) $ (fkOnColumn "user_group_id" "user_groups" "id") { fkOnDelete = ForeignKeySetNull }
      ]
}
...
, mgrAction = StandardMigration $ runQuery_ $ do
    ...
    sqlAlterTable tname [
      ...
      , sqlAddValidFK  tname $ (fkOnColumn "company_id" "companies" "id") { fkOnDelete = ForeignKeyNoAction }
