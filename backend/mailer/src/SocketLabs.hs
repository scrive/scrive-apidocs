                ...
                case mevent of
                  ...
                  Just event -> do
                    ...
                    if not res
                      ...
                      else logInfo "Event received" $ object ["event" .= show event]
          (mid, token) -> logInfo "Invalid id or token received"
            $ object [identifier mid, "token" .= fmap show token]
      _ -> do
        getField "Type" >>= \case
          Just "Validation" -> logInfo_ "Validation request received"
          _                 -> logInfo "Invalid MessageId received"
            $ object ["message_id" .= fmap show messageId]
...
withCallbackValidation cvks handler = do
  ...
  case msc of
    ...
    Just sc -> case findValidationKey cvks sc of
      Nothing -> do
        logAttention "SecretKey missing for SocketLabs callback"
          $ object ["secret_key" .= show sc]
