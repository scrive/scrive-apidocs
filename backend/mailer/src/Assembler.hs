  ...
  let -- FIXME: add =?UTF8?B= everywhere it is needed here
      ...
      headerMixed Attachment {..} =
        ...
        in  "\r\n--"
              ...
              <> "Content-Transfer-Encoding: base64\r\n"
              <> "\r\n"
      (relatedAtt, mixedAtt) = partition
        (\att -> ".png" `T.isSuffixOf` attName att || ".jpg" `T.isSuffixOf` attName att)
        mailAttachments
...
mailHeader headerName headerValue =
  prefix <> mailEncode (Just $ 75 - T.length prefix) headerValue <> "\r\n"
...
mailEncode mFirstLineLength source
  | asciiPrintable source = asciiMailEncode mFirstLineLength source
...
asciiMailEncode mFirstLineLength source = T.concat . intersperse "\r\n " $ map
  ...
  where
    ...
    aux
      :: [[Text]] -- list of lines generated so far (line is a list of words)
    ...
    aux ls l rsil (w : ws)
      | T.length w >= lineLength = -- this word will never fit with anything else
                                   case l of
...
createAddrString Address {..}
  | asciiPrintable addrName
  = mailEncode Nothing escapedName <> " <" <> punyEncode addrEmail <> ">"
  | otherwise
  = mailEncode Nothing addrName <> " <" <> punyEncode addrEmail <> ">"
