...
messengerTables = [tableMessengerWorkers, tableMessengerJobs, tableSMSes, tableSMSEvents]
  ...
  , tblInitialSetup =
    Just $ TableInitialSetup
      ...
      , initialSetup      = forM_ jobs $ \job -> do
                              runSQL_
                                $   "INSERT INTO messenger_jobs (id, run_at) VALUES ("
                                <?> job
                                <>  ", to_timestamp(0))"
      }
  }
...
tableSMSEvents = tblTable
  ...
  , tblPrimaryKey  = pkOnColumn "id"
  , tblForeignKeys = [ (fkOnColumn "sms_id" "smses" "id") { fkOnDelete = ForeignKeyCascade
                                                          }
                     ]
