        ...
        case mEvent of
          Just event -> do
            let updateEvent = do
                  ...
                  logInfo "UpdateWithSMSEventForTeliaID returned"
                    $ object ["telia_id" .= xID, "event" .= show event, "result" .= res]
            let
              handleMissingSMS (e :: DBException) = do
                logInfo
                    "UpdateWithSMSEventForTeliaID failed \
                    \(probably original SMS got purged)"
                  $ object
                      ["telia_id" .= xID, "event" .= show event, "exception" .= show e]
            updateEvent `E.catch` handleMissingSMS
            ok $ toResponse "<DeliveryResponse ack=\"true\"/>"
          Nothing -> do
            logInfo
                "UpdateWithSMSEventForTeliaID not run: \
              \could not read StatusCode"
              $ object ["status_code" .= xStatusCode]
