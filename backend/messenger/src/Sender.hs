  ...
  -- Telia CallGuide doesn't want leading +
    ...
    let teliaid =
          case
              take 1
              . drop 1
              . dropWhile (not . tagOpenNameLit "messageId")
              . parseTags
              $ resp
            of
              (TagText tid) : _ -> tid
...
curlSMSSender
  :: (CryptoRNG m, MonadBase IO m, MonadIO m, MonadLog m)
  => [Text]
  -> Text
  -> m (Bool, Text)
curlSMSSender params msisdn = do
  logInfo_ $ showt params
  (code, stdout, stderr) <- readCurl
    ((T.unpack <$> params) <> ["--write-out", "\n%{http_code}"])
    BS.empty
...
logInfoSendSMS sender sm =
  logInfo "Sending SMS" $ object ["sender" .= sender, logPair_ sm]
