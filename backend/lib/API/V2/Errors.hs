...
endpointNotFound ep =
  APIError { errorType = EndpointNotFound
           , errorHttpCode = 404
           , errorMessage = msg }
  where msg = "The endpoint" <+> ep <+>
              "was not found. See our website for API documentation"

-- | Used interally by this module and API.V2.User
invalidAuthorization :: APIError
invalidAuthorization =
  APIError { errorType = InvalidAuthorization
           , errorHttpCode = 401
           , errorMessage = msg}
  where msg = "No valid access credentials were provided. \
...
insufficientPrivileges =
  APIError { errorType = InsufficientPrivileges
           , errorHttpCode = 403
           , errorMessage = msg }
  where msg = "The access credentials provided do not have \
...
requestParameterInvalid param reason =
  APIError { errorType = RequestParametersInvalid
           ...
           , errorMessage = msg }
  where msg = "The parameter '" <> param <>
              "' had the following problems:" <+> reason

-- Document calls errors

documentObjectVersionMismatch :: DocumentObjectVersionDoesNotMatch -> APIError
documentObjectVersionMismatch (DocumentObjectVersionDoesNotMatch {..}) =
  APIError { errorType = DocumentObjectVersionMismatch
           , errorHttpCode = 409
           , errorMessage = msg }
...
documentActionForbidden =
  APIError { errorType = DocumentActionForbidden
           , errorHttpCode = 403
           , errorMessage = msg }
...
convertDocumentDoesNotExist (SomeDBExtraException ex) =
  case cast ex of
    Just (DocumentDoesNotExist did)
      -> SomeDBExtraException $ documentNotFound did
    Nothing
...
convertUserShouldBeSelfOrCompanyAdmin (SomeDBExtraException ex) =
  case cast ex of
    Just (UserShouldBeSelfOrCompanyAdmin{})
...
convertSignatoryRoleIsNotSigningParty (SomeDBExtraException ex) =
  case cast ex of
    Just (SignatoryRoleIsNotSigningParty {})
      -> SomeDBExtraException . signatoryStateError $
      "Signatory should not sign this document"
    Nothing
      -> SomeDBExtraException ex
...
convertSignatoryTokenDoesNotMatch (SomeDBExtraException ex) =
  case cast ex of
    Just (SignatoryTokenDoesNotMatch {})
      -> SomeDBExtraException . invalidAuthorizationWithMsg $
         "Signatory token does not match"
    Nothing
      -> (SomeDBExtraException ex)
...
convertSignatoryAuthenticationToSignDoesNotMatch (SomeDBExtraException ex) =
  case cast ex of
    Just (SignatoryAuthenticationToSignDoesNotMatch {})
      -> SomeDBExtraException . signatoryStateError $
         "Invalid authorization for signatory"
    Nothing
      -> SomeDBExtraException ex
