...
processEvents
  :: (MonadDB m, MonadReader CronEnv m, MonadLog m, MonadCatch m, CryptoRNG m)
  => Int
  -> m Int
  ...
  where
    processEvent (eid, mid, eventType, kontraInfoForMail) = do
      ...
      case kontraInfoForMail of
        DocumentInvitationMail docid slid ->
          forExistingDocument docid
            $ handleEventInvitation slid timeDiff templates eventType mailNoreplyAddress
        DocumentConfirmationMail docid slid ->
          forExistingDocument docid $ handleEventConfirmation slid
                                                              timeDiff
                                                              templates
                                                              eventType
                                                              mailNoreplyAddress
        OtherDocumentMail docid ->
          forExistingDocument docid $ handleEventOtherMail timeDiff eventType
...
markEventAsRead eid = do
  ...
  unless success $ logAttention_ "Couldn't mark event as read"

logEmails :: MonadLog m => Text -> Text -> m ()
logEmails signemail email = logInfo "Comparing emails"
  $ object ["signatory_email" .= signemail, "event_email" .= email]
...
handleEventInvitation slid timeDiff templates eventType mailNoreplyAddress =
  logSignatory slid $ do
    ...
    theDocument >>= \doc -> runTemplatesT (getLang doc, templates) $ case nev of
      ...
      DeliveryEvent Undelivered -> handleUndeliveredInvitation mailNoreplyAddress bd slid
      DeliveryEvent Deferred -> handleDeferredInvitation mailNoreplyAddress bd slid email
      _ -> return ()

handleEventConfirmation
  :: (CryptoRNG m, DocumentMonad m, MonadCatch m, MonadLog m, MonadThrow m, MonadDB m)
  => SignatoryLinkID
  -> Maybe Int
  -> GlobalTemplates
  -> Event
...
handleEventConfirmation slid timeDiff templates eventType mailNoreplyAddress =
  logSignatory slid $ do
    ...
    logEmails signemail email
    theDocument >>= \doc -> runTemplatesT (getLang doc, templates) $ case nev of
      DeliveryEvent Delivered -> handleDeliveredConfirmation slid timeDiff
...
handleEventOtherMail
  :: (DocumentMonad m, MonadLog m, MonadThrow m, MonadDB m) => Maybe Int -> Event -> m ()
...
handleEventLoggingDeliveryTime
  :: (DocumentMonad m, MonadLog m, MonadThrow m) => Text -> Maybe Int -> Event -> m ()
...
logDeliveryTime timeDiff = theDocument >>= \d -> do
  logInfo "Email delivered"
    $ object [identifier $ documentauthorugid d, "delivery_time" .= timeDiff]
...
handleUndeliveredInvitation
  :: (CryptoRNG m, MonadCatch m, MonadLog m, DocumentMonad m, TemplatesMonad m)
  => Text
  -> BrandedDomain
  -> SignatoryLinkID
  -> m ()
