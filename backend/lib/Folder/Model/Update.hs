...
instance (MonadDB m, MonadThrow m) => DBUpdate m FolderUpdate () where
  update (FolderUpdate newFolder) = do
    ...
    when (fid `elem` newParentPath) $
      throwM . SomeDBExtraException . FoldersFormCycle $ fid
    -- update folder
    ...
    where arraySliceCmd oldParentPath newParentPath =
            ("array_cat(parent_path[ 1:" <>
             "(array_length(parent_path, 1) - " <?> length oldParentPath <+> ")]" <>
