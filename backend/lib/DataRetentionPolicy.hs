...
makeStricterDataRetentionPolicy :: DataRetentionPolicy -> DataRetentionPolicy
                                -> DataRetentionPolicy
makeStricterDataRetentionPolicy drp1 drp2 = DataRetentionPolicy
    ...
    , _drpImmediateTrash            =
        get drpImmediateTrash drp1 || get drpImmediateTrash drp2
    }
...
unjsonDataRetentionPolicy = objectOf $ DataRetentionPolicy
  <$> fieldOpt "idle_doc_timeout_preparation" (get drpIdleDocTimeoutPreparation)
               "Number of days before moving documents in preparation to trash"
  <*> fieldOpt "idle_doc_timeout_closed" (get drpIdleDocTimeoutClosed)
               "Number of days before moving closed documents to trash"
  <*> fieldOpt "idle_doc_timeout_canceled" (get drpIdleDocTimeoutCanceled)
               "Number of days before moving cancelled documents to trash"
  <*> fieldOpt "idle_doc_timeout_timedout" (get drpIdleDocTimeoutTimedout)
               "Number of days before moving timed out documents to trash"
  <*> fieldOpt "idle_doc_timeout_rejected" (get drpIdleDocTimeoutRejected)
