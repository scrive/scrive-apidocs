...
addImageToDocumentFile documentid file@File{fileid} image pageno x y =
  withSystemTempDirectory' ("stamp-" ++ show documentid ++ "-" ++ show fileid ++ "-") $ \tmppath -> do
...
addImageSpecForDocument ::
  ( MonadIO m, TemplatesMonad m, MonadDB m, MonadMask m, MonadLog m
  , MonadFileStorage m , MonadBaseControl IO m )
  => String -> DocumentID -> File -> Int32 -> Double -> Double
  -> m AddImageSpec
addImageSpecForDocument inputpath did imageFile pageno x y = do
  imageContent <- getFileContents imageFile
  return $ AddImageSpec
    { addImageInput              = inputpath
