...
signatoryIdentifier :: SignatoryIdentifierMap -> SignatoryLinkID -> Text
                    -> Maybe Text
signatoryIdentifier sim slid emptyNamePlaceholder = do
  si <- Map.lookup slid sim
  return $ (if T.null (siFullName si)
...
signatoryIdentifierMap includenonsigning docs slids =
  ...
  where
    ...
    initials      = uniqueStrings $ enumerateEmpty $
                       map (T.concat . map (T.take 1) . T.words) names
...
uniqueStrings ss = snd . mapAccumL (go 1) Set.empty $ ss where
  ...
  go n used s | normalize sn `Set.member` used = go (n+1) used' s
              | otherwise          = (used', sn)
    where sn | n > 1 || Map.lookup (normalize s) ssOccurrences > Just 1 = s <> (showt n) -- Attempt to only append "1" if there are more occurrences down the list.
             | otherwise                                                = s
