...
unjsonSignatoryConsentModule = objectOf $ (,)
    <$> fieldOpt "title" fst "Consent module title"
    <*> fieldBy "questions" snd "Consent module questions" (arrayOf unjsonQuestion)
  ...
  where
    ...
    unjsonDescription = objectOf $ (,)
      <$> field "title" fst "Description title"
      <*> field "text"  snd "Description contents"
...
instance FromSQL SignatoryConsentResponsesForSigning where
  ...
  fromSQL mbase = do
    ...
    case Aeson.eitherDecode s of
      Left _ -> hpqTypesError $ "fromSQL (SignatoryConsentResponsesForSigning): can't parse json"
      Right ae -> case parse unjsonSignatoryConsentResponsesForSigning ae of
        (Result res []) -> return res
        (Result _ _) -> hpqTypesError "fromSQL (SignatoryConsentResponsesForSigning): can't parse SignatoryConsentResponsesForSigning"
...
instance ToSQL SignatoryConsentResponsesForSigning where
  ...
  toSQL s = toSQL (unjsonToByteStringLazy' (Options { pretty = False, indent = 0, nulls = True }) unjsonSignatoryConsentResponsesForSigning s)

unjsonSignatoryConsentResponsesForSigning :: UnjsonDef SignatoryConsentResponsesForSigning
unjsonSignatoryConsentResponsesForSigning =
    unjsonInvmapR (flip Result [] . SignatoryConsentResponsesForSigning)
                  (\(SignatoryConsentResponsesForSigning xs) -> xs)
                  (arrayOf unjsonResponse)
  where
    unjsonResponse :: UnjsonDef (SignatoryConsentQuestionID, Bool)
    unjsonResponse = objectOf $ (,)
      <$> field "id" fst "Question ID"
      <*> field "response" snd "Response"
