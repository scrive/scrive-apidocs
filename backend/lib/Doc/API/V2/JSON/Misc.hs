...
utcTimeToAPIFormat time =
  T.pack (formatTime defaultTimeLocale "%Y-%m-%dT%H:%M:%S%5Q" time) <> "Z"

-- Unjson for few simple enum types used
unjsonDocumentStatus :: UnjsonDef DocumentStatus
unjsonDocumentStatus = unjsonEnumBy
  "DocumentStatus"
...
unjsonSignatoryRole = unjsonEnumBy
  "SignatoryRole"
...
unjsonDeliveryStatus = unjsonEnumBy
  "DeliveryStatus"
  [ (Delivered  , "delivered")
...
unjsonAuthenticationToSignMethod = unjsonEnumBy
  ...
  , (LegacyDKNemIDAuthenticationToSign      , "dk_nemid")
  , (DKNemIDCPRAuthenticationToSign         , "dk_nemid_cpr")
  , (DKNemIDPIDAuthenticationToSign         , "dk_nemid_pid")
  , (DKNemIDCVRAuthenticationToSign         , "dk_nemid_cvr")
...
unjsonDeliveryMethod = unjsonEnumBy
  "DeliveryMethod"
  [ (EmailDelivery         , "email")
  , (PadDelivery           , "pad")
  , (APIDelivery           , "api")
...
unjsonConfirmationDeliveryMethod = unjsonEnumBy
...
unjsonNotificationDeliveryMethod = unjsonEnumBy
...
unjsonAuthorAttachment =
  objectOf
    ...
    <*> field "add_to_sealed_file"
              authorattachmentaddtosealedfile
              "Should attachmnet be added to sealed file"
...
unjsonSignatoryAttachment =
  objectOf
    ...
    <*> fieldDef "required"
                 True
                 signatoryattachmentrequired
                 "Uploading attachment is mandatory"
...
unjsonScreenshots =
  ...
  where
    ...
    parseScreenshot _ =
      fail "Can't parse screenshot image from something that is not string"
...
unjsonSignatoryScreenshots =
  ...
  where
    ...
    combineSignatoryScreenshots f s _ (Just rs) =
      SignatoryScreenshots.SignatoryScreenshots f s (Just $ Right rs)
    combineSignatoryScreenshots f s _ _ =
      SignatoryScreenshots.SignatoryScreenshots f s Nothing
