...
applyDraftDataToDocument draft actor = do
  ...
  unlessM (isPreparation <$> theDocument) $ do
    theDocument
      >>= \doc -> logInfo "API V2 - Document is not in preparation" $ logObject_ doc
    apiError $ serverError
      "Could not apply draft data to document as document is not in preparation."
      ...
      case enop of
        ...
        Right numOfPages -> do
          ...
          let
            ...
            clearField (SignatoryCompanyField scf) = SignatoryCompanyField
              $ scf { scfPlacements = filter validPlacement $ scfPlacements scf }
            clearField (SignatoryPersonalNumberField spnf) =
              SignatoryPersonalNumberField
                $ spnf { spnfPlacements = filter validPlacement $ spnfPlacements spnf }
            clearField (SignatoryCompanyNumberField scnf) =
              SignatoryCompanyNumberField
                $ scnf { scnfPlacements = filter validPlacement $ scnfPlacements scnf }
            clearField (SignatoryEmailField sef) = SignatoryEmailField
              $ sef { sefPlacements = filter validPlacement $ sefPlacements sef }
            clearField (SignatoryMobileField smf) = SignatoryMobileField
              $ smf { smfPlacements = filter validPlacement $ smfPlacements smf }
            clearField (SignatoryTextField stf) = SignatoryTextField
              $ stf { stfPlacements = filter validPlacement $ stfPlacements stf }
            clearField (SignatoryCheckboxField schf) = SignatoryCheckboxField
              $ schf { schfPlacements = filter validPlacement $ schfPlacements schf }
            clearField (SignatorySignatureField ssf) = SignatorySignatureField
              $ ssf { ssfPlacements = filter validPlacement $ ssfPlacements ssf }
            clearField (SignatoryRadioGroupField srgf) = SignatoryRadioGroupField
              $ srgf { srgfPlacements = filter validPlacement $ srgfPlacements srgf }
