      ...
      Right newdoc -> do
        ...
        case md of
          ...
          Just d  -> do
            ...
            void $ withDocument d $ update $ InsertEvidenceEvent RestartDocumentEvidence
                                                                 (return ())
                                                                 actor
...
archiveIdleDocuments now = do
  ...
  where
    ...
    expireUserDocumentsInGroupsWithDRP = do
      ...
      let ugs_settings = mapMaybe (\ug -> (ug, ) <$> ugSettings ug) ugs
      counts_and_ugids <- forM ugs_settings $ \(ug_with_drp, ug_settings) -> do
        let ug_drp = ugsDataRetentionPolicy ug_settings
        ...
        children_ugs <- ugGetChildrenInheritingProperty (ugID ug_with_drp)
                                                        ugSettings
        archived_counts <- forM (ug_with_drp : children_ugs) $ \ug_with_inherited_drp ->
          do
            users <- dbQuery $ UserGroupGetUsersIncludeDeleted $ ugID ug_with_inherited_drp
            forM users $ expireUserDocuments (Just ug_drp)
        return
          (sum $ concat archived_counts, map ugID $ ug_with_drp : children_ugs)
...
assertEqualDocuments d1 d2
  ...
  where
    ...
    eqMaybeSignInfo = liftEqMaybe $ \si1 si2 ->
      and [signtime si1 `eqUtcTime` signtime si2, signipnumber si1 == signipnumber si2]
