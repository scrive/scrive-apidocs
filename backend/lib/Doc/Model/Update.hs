...
insertAsAdditionalSignatoryLink did sl = do
  slid <- head <$> insertSignatoryLinksOnly
    did
    [(sl { signatorylinkid = unsafeSignatoryLinkID 0, signatoryisauthor = False })]
...
insertSignatoryLinks
  :: (MonadDB m, MonadLog m, MonadThrow m) => DocumentID -> [SignatoryLink] -> m ()
...
insertSignatoryLinks did links = do
  ...
  case filter signatoryisauthor linksWithID of
    [author] -> do
      runQuery_ . sqlUpdate "documents" $ do
        ...
        sqlWhereEq "id" did
    authors -> do
      logAttention "Document doesn't have exactly one author"
        $ object [identifier did, identifier $ map signatorylinkid authors]
  ...
  insertSignatoryConsentQuestions
    [ (signatorylinkid sl, cq)
    | sl <- linksWithID
    , cq <- signatorylinkconsentquestions sl
    ]


insertSignatoryLinksOnly
  :: MonadDB m => DocumentID -> [SignatoryLink] -> m [SignatoryLinkID]
insertSignatoryLinksOnly _   []    = return []
insertSignatoryLinksOnly did links = do
  runQuery_ . sqlInsert "signatory_links" $ do
    sqlSet "document_id" did
    sqlSetListWithDefaults "id" $ map
      (\sl -> if unsafeSignatoryLinkID 0 == signatorylinkid sl
        then Nothing
        else Just $ signatorylinkid sl
      )
      links
...
insertSignatoryAttachments
  :: MonadDB m => [(SignatoryLinkID, SignatoryAttachment)] -> m ()
...
insertSignatoryLinkFields xs = do
  runQuery_ . sqlInsert "signatory_link_fields" $ do
    ...
    sqlSetList "radio_button_group_values"
      $ map (fmap Array1 . fieldRadioGroupValues) fields
    sqlSetList "editable_by_signatory" $ map fieldEditableBySignatory fields
    sqlSetList "custom_validation_pattern"
      $ map (fmap tcvPattern . fieldTextCustomValidation) fields
    sqlSetList "custom_validation_positive_example"
      $ map (fmap tcvPositiveExample . fieldTextCustomValidation) fields
    sqlSetList "custom_validation_tooltip"
      $ map (fmap tcvTooltip . fieldTextCustomValidation) fields
    sqlResult "id"
...
insertDocumentTags
  :: MonadDB m => Bool -> DocumentID -> S.Set DocumentTag -> m (S.Set DocumentTag)
insertDocumentTags fetch did tags
  ...
  | otherwise = do
    ...
    if fetch then S.fromList <$> fetchMany toComposite else return S.empty
...
insertSignatoryScreenshots
  ...
  => [(SignatoryLinkID, SignatoryScreenshots)]
  -> m Int
insertSignatoryScreenshots l = do
  let (slids, types, times, ss) =
        unzip4
          $  f "first"     first
          <> f "signing"   signing
          <> f "reference" getReferenceScreenshot
      f col part =
        [ (slid, col, Screenshot.time s, Screenshot.image s)
...
insertSignatoryConsentQuestions
  :: MonadDB m => [(SignatoryLinkID, SignatoryConsentQuestion)] -> m ()
insertSignatoryConsentQuestions [] = return ()
insertSignatoryConsentQuestions questions =
  runQuery_ . sqlInsert "signatory_link_consent_questions" $ do
...
insertDocument :: (MonadLog m, MonadDB m, MonadThrow m) => Document -> m Document
...
insertNewDocument
  :: (MonadDB m, MonadThrow m, MonadLog m, CryptoRNG m) => Document -> m Document
insertNewDocument doc = do
  now       <- currentTime
  magichash <- random
  let docWithTime =
        doc { documentmtime = now, documentctime = now, documentmagichash = magichash }
  insertDocument docWithTime

-- Create new document based on existing one
newFromDocumentID
  :: (MonadDB m, MonadThrow m, MonadLog m, CryptoRNG m)
  => (Document -> Document)
  -> DocumentID
  -> m (Maybe Document)
...
newFromDocument
  :: (MonadDB m, MonadThrow m, MonadLog m, CryptoRNG m) => Document -> m (Maybe Document)
  ...
  dbUpdate (ChangeAuthenticationToViewMethod slid authKind newAuthToView mSSN mPhone actor)
    = do
      updateDocumentWithID . const $ do
        ...
        if authToViewNeedsPersonalNumber newAuthToView
          then do
            -- If the new AuthenticationToViewMethod needs a personal number, we
            -- either alter the existing field or add it to the signatory Maybe
            -- setting the value too along the way
            case ssnField of
            ...
            -- number, we check if the authentication to view method of the
            -- other kind or authentication to sign need it or if there are
            -- placements for this field. We set it to optional if none of the
            -- above is true.
            when
            ...
            $ do
                ...
                sqlWhereEq "signatory_link_id" slid
                sqlWhereEq "type"              PersonalNumberFT
        if authToViewNeedsMobileNumber newAuthToView
          then do
            -- If the AuthenticationToViewMethod needs a mobile number field,
            -- check if it exists for the signatory and make sure it is there,
            -- optionally also update with the value supplied
            ...
            -- When new authentication to view method doesn't need a mobile
            -- number, we check if the authentication to view method of the
            -- other kind or authentication to sign need it or if there are
            -- placements for this field. We set it to optional if none of the
            -- above is true.
      ...
      addChangeAuthenticationToViewEvidenceEvent sl (authtoviewfrom, authtoviewto) = do
        mapM_ (insertEvidenceFor sl) $ case authKind of
          ...
          AuthenticationToViewArchived ->
            authToViewArchivedChangeEvidence authtoviewfrom authtoviewto
    ...
    updateDocumentWithID . const $ do
      ...
      when (authToSignNeedsMobileNumber newAuthToSign) $ do
        ...
        case slMobileField of
          Just _ -> kRun1OrThrowWhyNot . sqlUpdate "signatory_link_fields" $ do
            forM_ mPhone (sqlSet "value_text")
            sqlSet "obligatory" True
            sqlWhereEq "signatory_link_id" slid
            sqlWhereEq "type"              MobileFT
          -- Note: default in table for `obligatory` is true
  ...
  dbUpdate (MarkDocumentSeen slid actor) = updateDocumentWithID $ \did -> do
    ...
    kRun1OrThrowWhyNotAllowIgnore . sqlUpdate "signatory_links" $ do
      ...
      sqlWhereExists . sqlSelect "documents" $ do
        ...
        sqlWhereDocumentStatusIsOneOf
          [Pending, Timedout, Canceled, DocumentError, Rejected]
...
instance (CryptoRNG m, MonadDB m, MonadThrow m, MonadLog m, TemplatesMonad m, MonadBase IO m) => DBUpdate m NewDocument Document where
  dbUpdate (NewDocument user title documenttype timezone nrOfOtherSignatories actor desiredFolderId)
    = do
      ...
      let authorlink0 = signLinkFromDetails' authorFields
                                             True
      ...
      let authorlink       = authorlink0 { maybesignatory = Just $ user ^. #id }
          ...
          othersignatories = replicate nrOfOtherSignatories $ signLinkFromDetails'
  ...
  dbUpdate (ForwardSigning sl message fieldsWithVTexts actor) = do
    ...
    where
      ...
      updateTextField (fi, t) (slf : slfs) =
        ...
        in  if fi == fieldIdentity slf then setTextValue t slf : rest else slf : rest
      ...
      Right newdoc -> do
        ...
        case md of
          ...
          Just d  -> do
            ...
            return $ Just d
      Left err -> do
        logAttention "Document restart failed"
          $ object [identifier $ documentid doc, "error" .= err]
        return Nothing
    where

      tryToGetRestarted = if documentstatus doc `notElem` [Canceled, Timedout, Rejected]
        then
          return $ Left
            ("Can't restart document with " <> showt (documentstatus doc) <> " status")
        else do
          Right <$> clearSignInfofromDoc

      clearSignInfofromDoc = do
        newSignLinks <- forM (documentsignatorylinks doc) $ \sl -> do
          let newFields = for (signatoryfields sl) $ \f -> case f of
          ...
          return $ defaultSignatoryLink
            ...
            , signatorylinknotificationdeliverymethod =
              signatorylinknotificationdeliverymethod sl
  ...
  dbUpdate (SignDocument slid mesig mpin screenshots actor) = do
    let
      ...
      sqlWhereAuthSign = case (mesig, mpin) of
        (Just (CGISEBankIDSignature_ _), _) ->
        ...
        (_, Just _) ->
          sqlWhereSignatoryAuthenticationToSignMethodIs SMSPinAuthenticationToSign -- We should check pin here, but for now we do it in controler
        (Nothing, Nothing) ->
          sqlWhereSignatoryAuthenticationToSignMethodIs StandardAuthenticationToSign
  ...
  dbUpdate (ResetSignatoryDetails signatories _actor) =
    updateDocumentWithID $ \documentid -> do
      ...
      case checkResetSignatoryData document signatories of
        ...
        errs -> do
          logAttention "Cannot reset signatory details on document"
            $ object [identifier documentid, "errors" .= errs]
          return False
...
instance (DocumentMonad m, TemplatesMonad m, MonadThrow m) => DBUpdate m UpdateFieldsForSigning () where
  ...
  dbUpdate (UpdateFieldsForSigning sl fields signaturesContent actor) =
    updateDocumentWithID . const $ do
      ...
      let
        updateValue :: (FieldIdentity, FieldValue) -> m ()
        updateValue (fieldIdent, newValue) = do
          ...
          let oldValue = case oldField of
                Just (SignatoryCheckboxField chf@CheckboxField{}) ->
                  BoolFV (schfValue chf)
                Just (SignatorySignatureField sf@SignatureField{}) ->
                  FileFV (ssfValue sf)
            ...
            void . dbUpdate $ InsertEvidenceEvent
              ...
              (do
                F.value "value" $ case newValue of
                  ...
                  FileFV   Nothing -> ""
                  FileFV (Just fi) ->
                    maybe "" imgEncodeRFC2397ToText (lookup fi signaturesContent)
                F.value "previousvalue" $ case oldValue of
                  ...
                  FileFV (Just fi) ->
                    maybe "" imgEncodeRFC2397ToText (lookup fi signaturesContent)
                when (emptyValue newValue) $ F.value "newblank" True
                when (emptyValue oldValue) $ F.value "prvblank" True
                case oldField of
                  ...
                  Just (SignatoryRadioGroupField f) -> do
                    ...
                    F.value "fieldvalues" . T.unpack $ T.intercalate
                      ", "
                      (map (\v -> "\"" <> v <> "\"") $ srgfValues f)
                  _ -> return ()
                case oldField of
  ...
  dbUpdate (UpdateConsentResponsesForSigning sl (SignatoryConsentResponsesForSigning responses) actor)
    = updateDocumentWithID . const . forM_ responses $ \(i, r) -> do
      ...
      case find ((== i) . scqID) (signatorylinkconsentquestions sl) of
        Nothing -> throwM . SomeDBExtraException $ SignatoryConsentQuestionDoesNotExist i
...
instance (DocumentMonad m, TemplatesMonad m, MonadThrow m) => DBUpdate m RemoveDocumentAttachments Bool where
  dbUpdate (RemoveDocumentAttachments fid _actor) = updateDocumentWithID $ \did -> do
    count <-
      ...
      <+> "AND file_id ="
      <?> fid
      <+> "AND EXISTS (SELECT 1 FROM documents WHERE id = author_attachments.document_id AND status ="
      <?> Preparation
      <+> ")"
    return $ count > 0
...
instance (DocumentMonad m) => DBUpdate m SetSigAttachments () where
  dbUpdate (SetSigAttachments slid sigatts _actor) = updateDocumentWithID . const $ do
    ...
    where
      doDeleteAll =
        runQuery $ "DELETE FROM signatory_attachments WHERE signatory_link_id =" <?> slid
...
instance (MonadDB m, MonadTime m) => DBUpdate m PurgeDocuments Int where
  dbUpdate (PurgeDocuments savedDocumentLingerDays) = do
    ...
    runQuery . sqlUpdate "documents" $ do
      ...
      sqlWith "purged_signatory_consent_questions"
        . sqlDelete "signatory_link_consent_questions"
        $ do
            sqlWhere "signatory_link_id IN (SELECT id FROM purged_signatory_links)"
...
-}
archiveIdleDocuments
  :: (MonadLog m, MonadDB m, MonadThrow m, MonadBase IO m) => UTCTime -> m Int
archiveIdleDocuments now = do
  ...
  where
    ...
    expireUserDocuments m_ug_drp user = do
      ...
      logInfo "archiveIdleDocuments starting for user in user_group"
...
-- Update utilities
getEvidenceTextForUpdateField
  :: SignatoryLink -> FieldIdentity -> CurrentEvidenceEventType
...
getEvidenceTextForUpdateField _ (NameFI (NameOrder _)) =
  unexpectedError "NameFT with nameorder different than 1 and 2"
...
hasOneNameField sig =
  1 == length (filter (\f -> NameFT == fieldType f) $ signatoryfields sig)

updateWithoutEvidence
  :: (DocumentMonad m, MonadThrow m, Show a, ToSQL a) => SQL -> a -> m Bool
updateWithoutEvidence col newValue = updateDocumentWithID $ \did -> do
  runQuery01
    $   "UPDATE"
    <+> raw (tblName tableDocuments)
    <+> "SET"
    <+> (col <+> "=" <?> newValue <+> "WHERE id =" <?> did)
...
checkEqualSignatoryFields
  :: Text -> [SignatoryField] -> [SignatoryField] -> Maybe (Text, Text, Text)
...
checkEqualByAllowSecondNothing
  :: TextShow b
...
assertEqualDocuments d1 d2
  | null inequalities = return ()
  | otherwise = do
    logInfo message
      $ object ["inequalities" .= T.concat (showInequality <$> inequalities)]
  ...
  where
    ...
    checkSigLink s1 s2 = map
      ...
      , checkEqualBy eqMaybeUtcTime
                     "signatorylinkreallydeleted"
                     signatorylinkreallydeleted
      , checkEqualBy (==) "signatorylinkcsvupload" signatorylinkcsvupload
      , \s1' s2' -> checkEqualSignatoryFields "signatoryfields"
                                              (signatoryfields s1')
                                              (signatoryfields s2')
    ...
    inequalities =
      catMaybes
        $  map
             ...
             , checkEqualBy (==)
                            "documentauthorattachments"
                            (sort . documentauthorattachments)
    ...
    eqMaybeSignInfo = liftEqMaybe $ \si1 si2 ->
      (signtime si1 `eqUtcTime` signtime si2) && (signipnumber si1 == signipnumber si2)
