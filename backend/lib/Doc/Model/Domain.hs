...
{-
  In the future we should handle document access mostly or exclusively through the folder
  and access control systems. The below mixes the folder approach and the legacy approach
  since we're in a transition phase. Ideally, `DocumentsVisibleToSigningPartyOrByFolders` should
  retrieve the same set of documents (for now at least) as `DocumentsVisibleToUser`, if
  the Folders generated by default were not changed.

  The old list calls return all documents, which the user can act on as SigningParty or
  they are author or authors admin.

  The new list calls return all documents, which the user can act on as SigningParty or
  the document is in one of the Folders (which correspond to the old behaviour for default
  Folder setup)

  `UserID` is included in the data constructor because of the SigningParty based access

-}
documentDomainToSQL (DocumentsVisibleToSigningPartyOrByFolders uid shared_fids started_fids author_fids) = do
  sqlJoinOn "signatory_links" "documents.id = signatory_links.document_id"
  sqlJoinOn "users" "signatory_links.user_id = users.id"
...
userIsSignatoryOrApproverAndHasAppropriateSignOrder :: (MonadState v m, SqlWhere v)
                                                    => UserID -> m ()
