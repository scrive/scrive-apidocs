...
signLinkFromDetails'
  ...
  -> [SignatoryAttachment]
  -> SignatoryLink
signLinkFromDetails' fields author role sorder attachments = defaultSignatoryLink
...
signatoryLinkClearField field = case field of
  SignatorySignatureField (sf@(SignatureField{})) ->
  ...
  where
    fillData sf = case sf of
      SignatoryNameField nf@(NameField { snfNameOrder = NameOrder 1 }) ->
        SignatoryNameField $ nf { snfValue = fstname }
      SignatoryNameField nf@(NameField { snfNameOrder = NameOrder 2 }) ->
        SignatoryNameField $ nf { snfValue = sndname }
      SignatoryNameField _ ->
        unexpectedError "Name field with order different than 1 or 2"
      SignatoryCompanyField cf@(CompanyField{}) ->
        SignatoryCompanyField $ cf { scfValue = company }
      SignatoryPersonalNumberField pnf@(PersonalNumberField{}) ->
        SignatoryPersonalNumberField $ pnf { spnfValue = personalnumber }
      SignatoryCompanyNumberField cnf@(CompanyNumberField{}) ->
        SignatoryCompanyNumberField $ cnf { scnfValue = companynumber }
      SignatoryEmailField cef@(EmailField{}) ->
        SignatoryEmailField $ cef { sefValue = email }
      SignatoryMobileField cmf@(MobileField{}) ->
        SignatoryMobileField $ cmf { smfValue = mobile }
      _ -> sf
...
replaceSignatoryUser siglink user ugwp = (replaceSignatoryData
  ...
  where
    getUGEntityName    = view #ugaEntityName . ugwpAddress
    getUGCompanyNumber = view #ugaCompanyNumber . ugwpAddress
