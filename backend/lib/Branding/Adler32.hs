...
brandingAdler32 :: (MonadDB m, MonadThrow m) => Context -> Maybe (UserGroupID, UserGroupUI) -> m Text
brandingAdler32 ctx mugidandui = do
  ...
  return $ adler32Text $ T.concat $
    [ ad1, ad2, ad3, showt versionID ]
...
userGroupUIAdler32 (ugid, ugui) = do
  themesMD5 <- dbQuery . GetThemesMD5 . catMaybes .
