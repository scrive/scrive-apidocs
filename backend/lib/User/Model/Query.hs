...
instance MonadDB m => DBQuery m UserGroupGetUsers [User] where
  dbQuery (UserGroupGetUsers ugid) = do
    runQuery_
      ...
      <+> "WHERE user_group_id ="
      <?> ugid
      <+> "AND deleted IS NULL ORDER BY email"
    fetchMany fetchUser
    ...
    where
      ...
      activeUsersQuery now =
        "SELECT parent_invoicing, user_group_id, count(user_id) FROM"
          <+> parenthesize
                (parenthesize (loggedInRecently now) <+> "UNION" <+> parenthesize
                  (docSentRecently now)
                )
      ...
      loggedInRecently now = toSQLCommand . sqlSelect "users u" $ do
      ...
      docSentRecently now = toSQLCommand . sqlSelect "users as u" $ do
          -- set up CTEs to get closest invoicing parent
        sqlWithClosestInvoicingID "closest_invoicing_parent"
...
instance (MonadDB m, MonadThrow m)
  ...
  dbQuery (IsUserDeletable user) = do
    n <- runQuery . sqlSelect "users" $ do
      ...
      sqlLimit 1
    if n == 0 then return Nothing else return $ Just UserNotDeletableDueToPendingDocuments
...
instance (MonadDB m, MonadTime m) => DBQuery m GetUsageStats [UserUsageStats] where
  dbQuery (GetUsageStats forWhom statsPartition interval) = do
    ...
    runQuery_ . sqlSelect (statsRelation <+> "JOIN users u ON (uid = u.id)") $ do
      ...
      forM_ chargeableItemKeys
        $ \(qName, _) -> sqlResult $ "COALESCE(" <> qName <> ", 0) AS" <+> qName
      sqlOrderBy "time_window DESC"
      forM_ chargeableItemKeys $ \(qName, _) -> sqlOrderBy $ qName <+> "DESC"
      sqlOrderBy "u.email"
    ...
    where
      ...
      fetchUserUsageStats
        ...
        -> UserUsageStats
      fetchUserUsageStats (time_window_start, user_email, user_name, docs_sent, docs_closed, sigs_closed, sms_sent, sms_sent_via_telia, se_bankid_sigs, se_bankid_auths, no_bankid_auths, nemid_auths, no_bankid_sigs, nemid_sigs, tupas_auths, shareable_links)
        = UserUsageStats
...
chargeableItemKeys =
  ...
  , ("se_bankid_sigs", CISEBankIDSignatureFinished)
  , ("se_bankid_auths", CISEBankIDAuthenticationFinished)
  , ("no_bankid_auths", CINOBankIDAuthenticationFinished)
  , ("nemid_auths", CIDKNemIDAuthenticationFinished)
  , ("no_bankid_sigs", CINOBankIDSignatureFinished)
  , ("nemid_sigs", CIDKNemIDSignatureFinished)
  , ("tupas_auths", CIFITupasAuthenticationFinished)
...
instance (MonadDB m, MonadTime m) => DBQuery m GetUsageStatsOnShareableLinks [ShareableLinkUsageStats] where
  dbQuery (GetUsageStatsOnShareableLinks forWhom statsPartition interval) = do
    ...
    where
      ...
      fetchShareableLinkUsageStats (time_window_start, template_id, template_title, docs_sent, docs_closed, sigs_closed, sms_sent, sms_sent_via_telia, se_bankid_sigs, se_bankid_auths, no_bankid_auths, nemid_auths, no_bankid_sigs, nemid_sigs, tupas_auths, shareable_links)
        = ShareableLinkUsageStats
...
startingDate now interval = dateTrunc (sqlParam now <+> "-" <?> interval)
...
sqlWithClosestInvoicingID publicName = do
  sqlWith "invoicing_parents"
    . sqlSelect
        (   "user_groups ug,"
        <+> "unnest(array_prepend(ug.id, ug.parent_group_path)) WITH ORDINALITY"
        )
    $ do
  ...
  sqlWith publicName . sqlSelect "invoicing_parents ip" $ do
    sqlJoinOn "closest_invoicing_parent_order_id cipoid"
              "ip.user_group_id = cipoid.user_group_id"
    sqlResult "ip.user_group_id user_group_id"
