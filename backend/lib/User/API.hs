...
setup2FA = V2.api $ do
  ...
  requiredPerm <- apiHasPermission $ canDo UpdateA . UserR $ user ^. #id
...
apiCallGetUserProfile = api $ do
  ...
  requiredPerm <- apiHasPermission $ canDo ReadA . UserR $ user ^. #id
  apiAccessControl user requiredPerm $ do
...
apiCallDeleteUser = V2.api $ do
  ...
  requiredPerm <- apiHasPermission $ canDo DeleteA . UserR $ user ^. #id
  apiAccessControl user requiredPerm $ do
...
apiCallGetTokenForPersonalCredentials uid = V2.api $ do
  -- Guards
  user         <- V2.getAPIUserWithAPIPersonal
  requiredPerm <- apiHasPermission $ canDo UpdateA $ UserR uid
  apiAccessControl user requiredPerm $ do
...
guardCanChangeUser adminuser otheruser = do
  requiredPerm <- apiHasPermission $ canDo UpdateA . UserR $ otheruser ^. #id
