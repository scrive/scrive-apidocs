...
fetchCgiGrpTransaction
  :: (CgiGrpTransactionType, SignatoryLinkID, Maybe Text, Text, Text, SessionID)
  -> CgiGrpTransaction
fetchCgiGrpTransaction (cgitt, slid, mttbs, transaction_id, order_ref, session_id) =
  case cgitt of
    ...
    CgiGrpSign -> CgiGrpSignTransaction
      slid
      (fromMaybe (unexpectedError "CGI Transaction field has NULL as text_to_be_signed")
                 mttbs
      )
      transaction_id
      order_ref
