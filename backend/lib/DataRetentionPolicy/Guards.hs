...
guardThatDataRetentionPolicyIsValid :: Kontrakcja m => DataRetentionPolicy
                                    -> Maybe DataRetentionPolicy -> m ()
guardThatDataRetentionPolicyIsValid drp mParentDRP = do
  ...
  when (fmap (get drpImmediateTrash) mParentDRP == Just True
        && not (get drpImmediateTrash drp)) $
    apiError $ requestParameterInvalid "immediate_trash" $
