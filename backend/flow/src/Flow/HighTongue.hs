...
parseSetExpect = withObject "expect" $ \o ->
    Set.fromList <$> traverse (uncurry parseExpect) (HM.toList o)
  where
    parseExpect k v = case k of
        ...
        _ -> typeMismatch "Expected one of the keys `approved-by`, `received-data`, `signed-by`, `viewed-by`" v
