...
def create_document(base_url, session, auth_header, parties):
    ...
    resp = post(session,
        ...
        headers=auth_header)
    resp = post(session,
        base_url + "/api/v2/documents/{document_id}/start".format(document_id=document_id),
        headers=auth_header)
    signatory = resp.json()['parties'][1]
    signatory_id = signatory['id']
    signatory_url = signatory['api_delivery_url']
    print("Signatory URL: {url}".format(url=(base_url + signatory_url)))

    user_session = req.Session()
    get(user_session, base_url + signatory_url)
    data = {"xtoken": user_session.cookies["xtoken"], "redirect": "https://google.com"}
    resp = post(user_session,
        base_url + "/eid-service/start/onfido/view/{document_id}/{signatory_id}".format(document_id=document_id, signatory_id=signatory_id),
        data=data)
