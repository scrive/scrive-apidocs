...
initialState' :: State
initialState' = TransducerState "initial" (Map.fromList
...
inputSet2 = Set.fromList
    [ EventInfo Signature "user1" "document3"
    , EventInfo Signature "user2" "document3"
    ]

machine' :: Machine
machine' = Transducer
    { _states = Map.fromList
        [ ( "initial", initialState')
        , ( "foo", TransducerState "foo" (Map.fromList
            [ (inputSet3, TransducerEdge inputSet3 [] "end")
            ]
            )
          )
        , ( "bar", TransducerState "bar" (Map.fromList
            [ (inputSet3, TransducerEdge inputSet3 [] "end")
            ]
            )
          )
...
testUnknownEvent = runAggregatorStep input4 initialAggregatorState @?=
...
testFullTransitionDifferentOrder = do
    ...
    -- Event 2
    let (result, nextState2@AggregatorState{..}) =
            runAggregatorStep input3 nextState1
    result @?= Left NeedMoreEvents
    receivedEvents @?= Set.fromList [input3, input2]
    -- Event 3
    let (result, _nextState3@AggregatorState{..}) =
            runAggregatorStep input1 nextState2
    result @?= Right (TransducerEdge inputSet1 [] "foo")
...
testFullTransitionInterleavedEvents = do
    ...
    -- Event 2
    let (result, nextState2@AggregatorState{..}) =
            runAggregatorStep input5 nextState1
    result @?= Left NeedMoreEvents
    ...
    let (result, _nextState4@AggregatorState{..}) =
            runAggregatorStep input1 nextState3
    result @?= Right (TransducerEdge inputSet1 [] "foo")

tests :: Test
tests = testGroup "Aggregator"
  ...
  , testCase "Test full transition with different order"
        testFullTransitionDifferentOrder
