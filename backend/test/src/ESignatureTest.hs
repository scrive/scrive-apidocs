...
eSignatureTests env = testGroup
  "E-signature"
  [ testThat "can perform operations on CGI Swedish BankID signatures in the database"
             env
             testCGISEBankIDSignatures
  ]
...
testCGISEBankIDSignatures = do
  ...
  let new_bids = bids { cgisebidsSignatoryName = cgisebidsSignatoryName bids <> "!" }
