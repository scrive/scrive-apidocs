...
testSignDocumentEvidenceLog = do
  ...
  addRandomDocument (rdaDefault author)
    ...
    , rdaSignatories =
        let signatory = OneOf
              [ AllOf [ RSC_AuthToSignIs StandardAuthenticationToSign
                      , RSC_IsSignatory
                      ]
              ]
...
testChangeAuthenticationToSignMethod = replicateM_ 10 $ do
  ...
  addRandomDocument (rdaDefault author)
    ...
    , rdaSignatories =
      ...
      assertJust $ find (\e -> evType e == Current ChangeAuthenticationToSignFromStandard) lg1
      ...
      assertEqual "Too many evidence logs for change authentication method to"
        (length $ filter (\e -> evType e == Current ChangeAuthenticationToSignFromStandard) lg2) 1
      assertEqual "Too many evidence logs for change authentication method from"
        (length $ filter (\e -> evType e == Current ChangeAuthenticationToSignToSMSPin) lg2) 1
...
testPurgeDocument = replicateM_ 10 $ do
  ...
  doc <- addRandomDocument (rdaDefault author)
    { rdaTypes = OneOf [Signable]
    , rdaStatuses = OneOf [Closed, Canceled, Rejected]
    }
  now <- currentTime
...
testUpdateConsentResponsesForSigningSuccess = do
  ...
  doc    <- addRandomDocument (rdaDefault author)
    ...
    , rdaStatuses = OneOf [Pending]
    , rdaSignatories =
      let authorLink = OneOf
            [ AllOf [ RSC_HasConsentModule True ]
            ]
...
testStatusClassSignedWhenAllSigned = replicateM_ 10 $ do
  ...
  doc <- addRandomDocument (rdaDefault author)
    ...
    , rdaSignatories =
        let signatory = OneOf
              [ AllOf [RSC_IsSignatoryThatHasSigned]
              ]
        in OneOf $ map (`replicate` signatory) [2..10]
    }
