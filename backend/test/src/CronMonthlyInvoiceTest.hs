...
cronMonthlyInvoiceTest env = testGroup "CronMonthlyInvoiceTest" $
  [ testThat "Monthly-invoice email created" env test
  ]

test :: TestEnv ()
test = do
  setTestTime unixEpoch
  runSQL_ $ "UPDATE cron_jobs SET run_at = '1970-01-01 00:00:00+00' WHERE id = 'monthly_invoice';"
  ...
  runSQL_ $ "SELECT COUNT(*) FROM mail_attachments JOIN mails ON (mails.id = mail_attachments.mail_id) WHERE mails.title = 'Monthly invoice'";
