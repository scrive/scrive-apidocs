...
configTests _ = testGroup "Configurations"
    [ testCase "kontrakcja-server config parses" $
      testConfRoundtrip @AppConf
      "AppConf" "configuration-templates/kontrakcja.conf.template"

    , testCase "cron config parses"              $
      testConfRoundtrip @CronConf
      "CronConf" "configuration-templates/cron.conf.template"

    , testCase "mailing-server config parses"    $
      testConfRoundtrip @MailingServerConf
      "MailingServerConf" "configuration-templates/mailing_server.conf.template"

    , testCase "messenger-server config parses"  $
      testConfRoundtrip @MessengerServerConf
      "MessengerServerConf"
      "configuration-templates/messenger_server.conf.template"

    , testCase "kontrakcja-server config template parses" $
      testConfTemplate @AppConf
      "configuration-templates/kontrakcja.conf.template"

    , testCase "cron config template parses"              $
      testConfTemplate @CronConf
      "configuration-templates/cron.conf.template"

    , testCase "mailing-server config template parses"    $
      testConfTemplate @MailingServerConf
