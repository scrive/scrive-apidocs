...
startableDocumentGen uid file =
  runOccurenceControl 0.8 $ startableDocumentOC uid file
...
startableDocumentOC uid file = do
  ...
  authorRole <- decide
    (pure SignatoryRoleApprover)
    (arbitrary `suchThat` (/= SignatoryRoleApprover))

  noSigningParty <- decide' pure
  notSigningRole <- liftGen $
    arbitrary `suchThat` (/= SignatoryRoleSigningParty)

  pure doc
    { documentmainfiles = if noMainfile then [] else [mainfile]
    , documentsignatorylinks =
        let firstSL:secondSL:sls' = sls
            author = firstSL
            ...
            sls''
              | noSigningParty =
                  map (overwriteSigningRole notSigningRole)
                      (author : secondSL : sls')
              | any ((== SignatoryRoleSigningParty) . signatoryrole) sls' =
                  author : secondSL : sls'
              | otherwise =
                  author
  ...
  where
    ...
    overwriteSigningRole role sl
      | signatoryrole sl == SignatoryRoleSigningParty =
          sl { signatoryrole = role }
      | otherwise = sl

startableDocumentStatusOC :: OccurenceControl DocumentStatus
startableDocumentStatusOC = do
  decide
    (elements [Pending, Closed, Canceled, Timedout, Rejected, DocumentError])
    (pure Preparation)
...
signatoryLinkOC = do
  ...
  authToView <-
    decide arbitrary (pure StandardAuthenticationToView)
  authToSign <-
    decide arbitrary (pure StandardAuthenticationToSign)

  placedFieldsForApprover <- decide' pure

  pure $
    let (mobileField', personalNumberField')
          | authToView == StandardAuthenticationToView
            && authToSign == StandardAuthenticationToSign =
        ...
        sl' = case ( missingFieldForDelivery
                   , signatorylinkdeliverymethod sl
                   ) of
          (Just True, EmailDelivery) -> sl
            { signatoryfields =
                filter (\case { SignatoryEmailField _ -> False; _ -> True })
                       allFields
            }
          ...
          (Just True, MobileDelivery) -> sl
            { signatoryfields =
                filter (\case { SignatoryMobileField _ -> False; _ -> True })
                       allFields
            }
        ...
        sl''
          ...
          | signatoryrole sl' == SignatoryRoleApprover =
              sl' { signatoryfields = map removePlacements (signatoryfields sl') }
          | otherwise = sl'
  ...
  where
    ...
    removePlacements = \case
      ...
      SignatoryCheckboxField f ->
        SignatoryCheckboxField f { schfPlacements = [] }
      SignatorySignatureField f ->
        SignatorySignatureField f { ssfPlacements = [] }
      SignatoryRadioGroupField f ->
        SignatoryRadioGroupField f { srgfPlacements = [] }
