...
screenshotTests _env = testGroup "ReferenceScreenshots" [
      testCase "Desktop reference screenshots are up to date" $ testRefScreenshotsUp2Date "standard"
    , testCase "Mobile reference screenshots are up to date" $ testRefScreenshotsUp2Date "mobile"
    ]
...
testRefScreenshotsUp2Date screenshotName = do
  ...
  case decode screenshotStr of
    Ok (JSObject obj) ->
      case find ((== "time") . fst) $ fromJSObject obj of
        Just (_, JSString s) -> case parseTimeISO (fromJSString s) of
                                 Just time -> if (28 `daysBefore` now) < time then
                                                 return ()
                                             else
                                                 assertFailure $ "Reference screenshot in " ++ screenshotName ++ ".json is too old"
