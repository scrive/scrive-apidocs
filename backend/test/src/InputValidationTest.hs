...
inputValidationTests _ = testGroup
  "InputValidation"
  [ testGroup
    "asValidEmail"
  ...
  , testGroup
    "asDirtyEmail"
    [ testProperty "lower cases" $ propDirtyEmailLowercases . withArbitraryUnicode
  ...
  , testGroup
    ...
    , testProperty "whitespace only is counted as empty"
                   propValidCompanyNameWhitespaceIsEmpty
  ...
  , testGroup
    ...
    , testProperty "whitespace only is counted as empty"
                   propValidPositionWhitespaceIsEmpty
    , testProperty "can only contain alphanumeric, spaces and chars &'():,-"
  ...
  , testGroup
    ...
    , testProperty "whitespace only is counted as empty"
                   propValidFieldValueWhitespaceIsEmpty
...
testValidEmailStripsWhitespace = do
  ...
  assert $ not (any (isWhitespace . fromGood) results)
  where
    surroundWithWhitespace :: Text -> Text
...
propDirtyEmailStripsWhitespace ws s =
  propStripWhitespace asDirtyEmail ws $ withArbitraryUnicode s
...
propValidNameStripsWhitespace ws ns =
  let xs = map nc ns in propStripWhitespace asValidName ws (T.pack xs)
...
propValidNameGoodExamples ns =
  let xs = map nc ns
  in  (not (null xs) && length xs < 50 && not (isWhitespace $ T.pack xs))
...
propValidCompanyNameStripsWhitespace ws ns =
  let xs = map cnc ns in propStripWhitespace asValidCompanyName ws $ T.pack xs
...
propValidCompanyNameGoodExamples ns =
  let xs = map cnc ns
  in  (not (null xs) && length xs < 100 && not (isWhitespace $ T.pack xs))
...
propValidCompanyNumberStripsWhitespace
  :: [WhitespaceChar] -> [CompanyNumberChar] -> Property
propValidCompanyNumberStripsWhitespace ws ns =
  let xs = map cn ns in propStripWhitespace asValidCompanyNumber ws $ T.pack xs
...
propValidCompanyNumberRestrictsChars = propJustAllowed
  asValidCompanyNumber
  [isUnicodeChar, isAlphaNum, isDigit, (`elem` ['-', ' '])]

propValidCompanyNumberGoodExamples :: [CompanyNumberChar] -> Property
propValidCompanyNumberGoodExamples ns =
  let xs = map cn ns
  in  (length xs > 4 && length xs < 15 && not (isWhitespace $ T.pack xs))
...
propValidAddressStripsWhitespace ws as =
  let xs = map ac as in propStripWhitespace asValidAddress ws $ T.pack xs
...
propValidAddressRestrictsChars = propJustAllowed
  asValidAddress
  [isUnicodeChar, isAlphaNum, (`elem` (' ' : " '():,/.#-"))]
...
propValidPositionStripsWhitespace ws ps =
  let xs = map pnc ps in propStripWhitespace asValidPosition ws $ T.pack xs
...
propValidPositionGoodExamples ps =
  let xs = map pnc ps
  in  (not (null xs) && length xs < 200 && not (isWhitespace $ T.pack xs))
...
propValidDocIDGoodExamples n = True ==> isGood . asValidDocID $ showt n
...
propValidFieldValueStripsWhitespace ws fs =
  let xs = map fvc fs in propStripWhitespace asValidFieldValue ws $ T.pack xs
...
propValidFieldValueRestrictsChars = propJustAllowed
  asValidFieldValue
  [isUnicodeChar, isAlphaNum, isPunctuation, isSymbol, (== ' ')]
...
testValidInviteTextBadExamplesAreFixed = do
  ...
  assert
    $  isGood e1
    && (T.filter (\c -> not $ isSpace c || isControl c) (fromGood e1) == "blah")
  let e2 = asValidInviteText "<p><BR></p>"
  assert
    $  isGood e2
    && (T.filter (\c -> not $ isSpace c || isControl c) (fromGood e2) == "")
...
propStripWhitespace f ws xs =
  let padding = T.pack $ map wc ws
      result  = f (padding <> xs <> padding)
  in  isGood result ==> isTrimmed . fromGood $ result

propWhitespaceIsEmpty :: (Text -> Result a) -> [WhitespaceChar] -> Property
propWhitespaceIsEmpty f xs = not (null xs) ==> isEmpty . f . T.pack . map wc $ xs
