...
apiV1CallsTests env =
  testGroup "CallsAPIV1"
    $  map (\d -> testThat (d <> " updates correctly") env (void $ testUpdateDoc d))
           jsonDocs
    <> [ testThat "settings auto reminder works"      env testSetAutoReminder
       ...
       , testThat "change main file moves placements with negative index"
                  env
                  testChangeMainFileMovePlacementsWithNegativeIndex
       , testThat "Changing authentication to view method works"
                  env
                  testChangeAuthenticationToViewMethod
       , testThat "Changing authentication to sign method works"
                  env
                  testChangeAuthenticationToSignMethod
       , testThat
         "Changing authentication to sign method without changing existing info works"
         env
         testChangeAuthenticationToSignMethodWithEmptyAuthenticationValue
       , testThat "Creating doc with access credentials via API works"
                  env
                  testOAuthCreateDoc
       , testThat "Creating doc with personal access credentials via API works"
                  env
                  testPersonalAccessCredentialsCreateDoc
...
jsonDocs :: [String]
jsonDocs = [inTestDir "json/document1.json", inTestDir "json/document2.json"]
...
testOAuthCreateDoc = do
  ...
  assertEqual "The OAuth API created document should be saved (DB)"
              False
              (documentunsaveddraft doc)
...
testSetAutoReminder = do
  ...
  let Just user = ctx ^. #ctxMaybeUser
...
testUpdateDocToSaved useOAuth = do
  ...
  let mkDocJSON d =
        runTestKontra reqDocJSON ctx $ documentJSONV1 (Just user) True True Nothing d

  reqDoc <- if useOAuth
    then mkRequestWithHeaders
      POST
      [("expectedType", inText "text"), ("file", inFile $ inTestDir "pdfs/simple.pdf")]
  ...
  when useOAuth $ do
    ...
    assertEqual "The API created document should be saved (DB)"
                False
                (documentunsaveddraft doc)
  unless useOAuth $ do
    assertEqual "The created document should not be saved (JSON)" False newSaved
    assertEqual "The created document should not be saved (DB)"
                True
                (documentunsaveddraft doc)
  ...
  assertEqual "Document should still be saved (DB)"
              False
              (documentunsaveddraft docUnsaved)
...
testChangeAuthenticationToViewMethod = do
  ...
  (resNoAuthMethod, _) <-
    runTestKontra reqNoAuthMethod ctx
  ...
  (resInvalidMethod, _) <-
    runTestKontra reqInvalidMethod ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  (resNOBankIDNoSSN, _) <-
    runTestKontra reqNOBankIDNoSSN ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  assertEqual "Response code should be 400" 400 (rsCode resNOBankIDNoSSN)

  reqNOBankIDValid <- mkRequest
    POST
    [ ("authentication_type", inText "no_bankid")
    , ("personal_number"    , inText "12345678901")
    ]
  (resNOBankIDValid, _) <-
    runTestKontra reqNOBankIDValid ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  assertEqual "Response code should be 202" 202 (rsCode resNOBankIDValid)

  reqNOBankIDValidWithMobile <- mkRequest
    POST
    [ ("authentication_type", inText "no_bankid")
    , ("personal_number"    , inText "12345678901")
    , ("mobile_number"      , inText "+4712345678")
    ]
  (resNOBankIDValidWithMobile, _) <-
    runTestKontra reqNOBankIDValidWithMobile ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  assertEqual "Authentication to view should be NOBankID"
              NOBankIDAuthenticationToView
              (signatorylinkauthenticationtoviewmethod siglinkNOBankID)
  assertEqual "The phone number +4712345678 should be set"
              "+4712345678"
              (getMobile siglinkNOBankID)
  assertEqual "The personal number 12345678901 should be set"
              "12345678901"
              (getPersonalNumber siglinkNOBankID)

  reqNOBankIDInvalidMobile <- mkRequest
    POST
    [ ("authentication_type", inText "no_bankid")
    , ("personal_number"    , inText "12345678901")
    , ("mobile_number"      , inText "+4612345678")
    ]
  (resNOBankIDInvalidMobile, _) <-
    runTestKontra reqNOBankIDInvalidMobile ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  (resSEBankIDButOldNOSSN, _) <-
    runTestKontra reqSEBankIDButOldNOSSN ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  assertEqual "Response code should be 400" 400 (rsCode resSEBankIDButOldNOSSN)

  reqSEBankIDValid12digits <- mkRequest
    POST
    [ ("authentication_type", inText "se_bankid")
    , ("personal_number"    , inText "123456789012")
    ]
  (resSEBankIDValid12digits, _) <-
    runTestKontra reqSEBankIDValid12digits ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  assertEqual "Authentication to view should be SEBankID"
              SEBankIDAuthenticationToView
              (signatorylinkauthenticationtoviewmethod siglinkSEBankID)
  assertEqual "The personal number 123456789012 should be set"
              "123456789012"
              (getPersonalNumber siglinkSEBankID)

  reqSEBankIDValid10digits <- mkRequest
    POST
    [ ("authentication_type", inText "se_bankid")
    , ("personal_number"    , inText "1234567890")
    ]
  (resSEBankIDValid10digits, _) <-
    runTestKontra reqSEBankIDValid10digits ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  (resStandard, _) <- runTestKontra reqStandard ctx
    $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
  ...
  assertEqual "The personal number 1234567890 should be set from previous call"
              "1234567890"
              (getPersonalNumber siglinkStandard)
  assertEqual "The mobile number +4712345678 should be set from previous call"
              "+4712345678"
              (getMobile siglinkStandard)
  assertEqual "Authentication to view should be Standard"
              StandardAuthenticationToView
              (signatorylinkauthenticationtoviewmethod siglinkStandard)

  -- Check that we can't change authentication to view for signatory that has already authenticated to view
  reqSEBankIDAgain <- mkRequest
    POST
    [ ("authentication_type", inText "se_bankid")
    , ("personal_number"    , inText "1234567890")
    ]
  (resSEBankIDAgain, _) <-
    runTestKontra reqSEBankIDAgain ctx
      $ apiCallV1ChangeAuthenticationToView (documentid doc) validsiglinkid
...
testChangeAuthenticationToSignMethod = do
  ...
  req <- mkRequest
    POST
    [ ("authentication_type" , inText "sms_pin")
    , ("authentication_value", inText "+46701234567")
    ]
...
testChangeAuthenticationToSignMethodWithEmptyAuthenticationValue = do
  ...
  assertEqual "The phone number +46701234567 should be set there"
              "+46701234567"
              (getMobile siglink)
  ...
  let updatedsiglinks' = documentsignatorylinks updatedDoc'
      siglink'         = head $ filter isSignatory updatedsiglinks'
  assertEqual "The phone number +46701234567 should be STILL there"
              "+46701234567"
              (getMobile siglink')
...
testChangeMainFileMovePlacementsWithNegativeIndex = do
  ...
  let updatejs = mapObjectEntry
      ...
      addAnchoredField (JSArray arr) = JSArray
        (  arr
        <> [ runJSONGen $ do
...
assertEqualDouble
  :: (MonadIO m) => String -> [(Int, Double, Double)] -> [(Int, Double, Double)] -> m ()
...
testCloseEvidenceAttachments = do
  ...
  let rspString                   = BS.toString $ rsBody rsp
      ...
      attachmentNames             = map
        (\(JSObject obj) ->
          let Just (JSString name) = lookup "name" $ fromJSObject obj
          in  fromJSString name
        )
        attachments
  assertEqual
    "Evidence Attachments file names and order do not match"
