...
import User.Types.User (UserInfo(..))

apiV2SetAuthenticationTests :: TestEnvSt -> Test
apiV2SetAuthenticationTests env = testGroup "APIv2DocumentPostCalls"
  [ testThat "API v2 Set signatory authentication-to-view"
             env testDocApiV2SetSignatoryAuthenticationToView
  , testThat "API v2 Set signatory authentication-to-view-archived"
             env testDocApiV2SetSignatoryAuthenticationToViewArchived
  , testThat "API v2 Set signatory authentication-to-sign"
             env testDocApiV2SetSignatoryAuthenticationToSign

  , testThat "API v2 Set viewer authentication-to-view"
             env testDocApiV2SetViewerAuthenticationToView
  , testThat "API v2 Set viewer authentication-to-view-archived"
             env testDocApiV2SetViewerAuthenticationToViewArchived
  , testThat "API v2 Set viewer authentication-to-sign"
             env testDocApiV2SetViewerAuthenticationToSign

  , testThat "API v2 Set approver authentication-to-view"
             env testDocApiV2SetApproverAuthenticationToView
  , testThat "API v2 Set approver authentication-to-view-archived"
             env testDocApiV2SetApproverAuthenticationToViewArchived
  , testThat "API v2 Set approver authentication-to-sign"
             env testDocApiV2SetApproverAuthenticationToSign
  ]
...
testDocApiV2SetSignatoryAuthenticationToView = do
  ...
  let param_auth x = ("authentication_type", inText x)
      ...
      setInvalidAuth = (\params expectedCode ->
          void $ jsonTestRequestHelper
            ctx
            POST
            params
  ...
  do -- Just to ensure limited scope so we don't test against the wrong thing
    ...
    assertEqual
      "NO Mobile number should be set"
      no_mobile
  ...
  do -- Just to ensure limited scope so we don't test against the wrong thing
    ...
    assertEqual
      "NO Mobile number should be empty"
      ""
...
testDocApiV2SetSignatoryAuthenticationToSign = do
  ...
  do -- Just to ensure limited scope so we don't test against the wrong thing
    ...
    assertEqual
      "Authentication to sign should be set"
  ...
  do -- Just to ensure limited scope so we don't test against the wrong thing
    ...
    assertEqual
      "Mobile number should not be set"
      ""
      (getMobileNumber mockDocSMSEmpty)
    assertEqual
      "Personal number should STILL be set (12 digit SE)"
      se_ssn_12
...
testDocApiV2SetViewerAuthenticationToView = do
  ...
  mockDoc    <- do
    ...
    let
      did = getMockDocId newMockDoc
      newParty = setMockDocSigLinkSignatoryRole SignatoryRoleViewer $
...
testDocApiV2SetApproverAuthenticationToView = do
  ...
  mockDoc    <- do
    ...
    let
      did = getMockDocId newMockDoc
      newParty = setMockDocSigLinkSignatoryRole SignatoryRoleApprover $
                 setMockDocSigLinkDeliveryMethod "pad" defaultMockSigLink
