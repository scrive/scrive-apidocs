...
signXMLDSig user_privkey_file user_cert_file tmpdirsuffix xml = do
  signed_xml_bs <- withSystemTempDirectory' ("XMLDSig-" ++ tmpdirsuffix ++ "-") $ \tmppath -> do
    ...
    let args =
          ...
          , "--privkey-pem", user_privkey_file ++ "," ++ user_cert_file
          , xml_file
          ]
...
wrapXMLDSigTmpl xml_content =
  documentA "Signature" [("xmlns", "http://www.w3.org/2000/09/xmldsig#")] $ do
    element "SignedInfo" $ do
      elementA "CanonicalizationMethod" [("Algorithm", "http://www.w3.org/TR/2001/REC-xml-c14n-20010315")] ()
      elementA "SignatureMethod" [("Algorithm", "http://www.w3.org/2000/09/xmldsig#rsa-sha1")] ()
      elementA "Reference" [("URI", "#object")] $ do
        elementA "DigestMethod" [("Algorithm", "http://www.w3.org/2000/09/xmldsig#sha1")] ()
        element "DigestValue" ()
