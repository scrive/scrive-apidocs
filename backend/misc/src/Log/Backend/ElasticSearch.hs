...
elasticSearchLogger ElasticSearchConfig{..} genRandomWord = do
    ...
    retryOnException . runBH_ $ do
      ...
      when (oldIndex /= index) $ do
        ...
        unlessM (indexExists index) $ do
          -- Bloodhound is incredibly stupid and won't let us create index using
          -- default settings, so pass these as they're fine according to Piotr.
          ...
          when (not $ isSuccess reply) $ do
            $unexpectedErrorM $ "ElasticSearch: error while creating mapping:" <+> T.unpack (T.decodeUtf8 . BSL.toStrict . jsonToBSL $ decodeReply reply)
  ...
  where
    ...
    checkElasticSearchConnection = try (void $ runBH_ listIndices) >>= \case
      Left (ex::HttpException) -> $unexpectedErrorM $ "ElasticSearch: unexpected error: " <> show ex <> " (is ElasticSearch server running?)"
      Right () -> return ()
