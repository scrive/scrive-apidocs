...
brandingPreview read state =
    if state.inherit then
        Maybe.withDefault defaultUserGroupBranding read.inheritableBranding

    else
        state.brandingBeingEdited


type Msg
    = SetThemeMsg ThemeKind (Maybe ThemeID)
...
update embed msg =
    ...
    case msg of
        SetThemeMsg kind mId ->
            updateBranding <| setTheme kind mId

        SetBrowserTitleMsg text ->
            updateBranding <| setBrowserTitle text

        SetSmsOriginatorMsg text ->
            updateBranding <| setSmsOriginator text

        OpenFaviconFileSelectMsg ->
            updateBranding <| openFaviconFileSelect embed

        LoadFaviconFileMsg file ->
            updateBranding <| loadFaviconFile embed file

        SetFaviconMsg contents ->
            updateBranding <| setFavicon contents

        SetInheritBrandingMsg inherit ->
            setInheritBranding inherit
...
setTheme kind mId state =
    let
        newThemes =
            case mId of
                Just id ->
                    Enum.insert kind id state.themes
...
viewThemeSelector : (Msg -> msg) -> ThemeKind -> EditUserGroupBrandingReadonly -> EditUserGroupBrandingState -> Form.Col msg
viewThemeSelector embed kind read state =
    let
        ...
        availableThemes =
            if state.inherit then
                read.inheritedThemes

            else
                read.userGroupThemes

        onSelect : String -> msg
        onSelect =
            embed << SetThemeMsg kind << String.toInt
        ...
        themeItem theme =
            Select.item
                ...
                , selected <|
                    case Enum.get kind branding.themes of
                        Just id ->
                            theme.id == id
...
viewEditUserGroupBranding :
    ...
    -> EditUserGroupBrandingReadonly
    -> EditUserGroupBrandingState
    -> Html msg
viewEditUserGroupBranding { embed, doSaveUserGroupBranding } read state =
    let
        branding =
            brandingPreview read state
    in
    div []
        [ Form.row []
            ...
            , Form.col [ Col.sm8, Col.md8, Col.lg8 ]
                ...
                , Form.help []
                    [ text <|
                        "Inherit all branding options from the "
        ...
        , Form.row []
            ...
            , Form.col [ Col.sm8, Col.md8, Col.lg8 ]
                [ div []
                    [ case branding.favicon of
                        Just favicon ->
                            img [ style "max-width" "50px", src favicon ] []
        ...
        , Form.row []
            ...
            , Form.col [ Col.sm8, Col.md8, Col.lg8 ]
                ...
                , Form.help []
                    [ text <|
                        "The name displayed to the recipient when "
