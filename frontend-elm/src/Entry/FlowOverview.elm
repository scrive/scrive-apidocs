...
maybeAdd ma b = M.map (\a -> ( a, b )) ma

getDocs : GetInstanceView -> Dict String Document ->
        ...
        AppOk { innerModel } ->
            case ( innerModel.mInstance, innerModel.mDocuments ) of
                ( Just instance, Just documents ) ->
                    let
                        ( actionDocsWithLinks, stateDocsNoLinks ) = getDocs instance documents
                        active = not <| List.isEmpty actionDocsWithLinks
                        statusHeader =
                            case instance.status of
                                InProgress -> "Document workflow is in progress"
                                Completed -> "Document workflow is complete"
                                Failed -> "Document workflow failed"
                        statusMessage =
                            case instance.status of
                                InProgress ->
                                    if active then
