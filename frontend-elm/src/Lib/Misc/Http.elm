...
encodeError error =
    case error of
        BadUrl str ->
            object [ ( "tag", string "BadUrl" ), ( "url", string str ) ]

        Timeout ->
            object [ ( "tag", string "Timeout" ) ]

        NetworkError ->
            object [ ( "tag", string "NetworkError" ) ]

        BadStatus code ->
            object [ ( "tag", string "BadStatus" ), ( "status", int code ) ]

        BadBody body ->
            object [ ( "tag", string "BadBody" ), ( "body", string body ) ]
