        ...
        |> JD.andThen
            (\s ->
                case s of
                    ...
                    "signing_party" ->
                        JD.succeed SigningParty

                    "forwarded_party" ->
                        JD.succeed ForwardedParty

                    _ ->
                        JD.fail <| "Cannot parse SignatoryRole: " ++ s
            )
    ...
    JD.string
        |> JD.andThen
            (\s ->
                case s of
                    "delivered" ->
                        JD.succeed Delivered

                    "not_delivered" ->
                        JD.succeed Undelivered
                    ...
                    _ ->
                        JD.fail <| "Cannot parse DeliveryStatus: " ++ s
            )
    ...
    JD.string
        |> JD.andThen
            (\s ->
                case s of
                    "email" ->
                        JD.succeed EmailDelivery

                    "pad" ->
                        JD.succeed PadDelivery

                    "api" ->
                        JD.succeed APIDelivery

                    "mobile" ->
                        JD.succeed MobileDelivery

                    "email_mobile" ->
                        JD.succeed EmailAndMobileDelivery

                    "portal" ->
                        JD.succeed PortalDelivery

                    _ ->
                        JD.fail <| "Cannot parse DeliveryMethod: " ++ s
            )
    ...
    JD.string
        |> JD.andThen
            (\s ->
                case s of
                    "standard" ->
                        JD.succeed StandardAuthenticationToView

                    "se_bankid" ->
                        JD.succeed SEBankIDAuthenticationToView

                    "no_bankid" ->
                        JD.succeed NOBankIDAuthenticationToView

                    "dk_nemid" ->
                        JD.succeed LegacyDKNemIDAuthenticationToView

                    "dk_nemid_cpr" ->
                        JD.succeed DKNemIDCPRAuthenticationToView

                    "dk_nemid_pid" ->
                        JD.succeed DKNemIDPIDAuthenticationToView

                    "dk_nemid_cvr" ->
                        JD.succeed DKNemIDCVRAuthenticationToView

                    "fi_tupas" ->
                        JD.succeed FITupasAuthenticationToView

                    "sms_pin" ->
                        JD.succeed SMSPinAuthenticationToView

                    "verimi" ->
                        JD.succeed VerimiAuthenticationToView

                    "nl_idin" ->
                        JD.succeed IDINAuthenticationToView
                    ...
                    _ ->
                        JD.fail <| "Cannot parse AuthenticationToViewMethod: " ++ s
            )
...
signatoryLinkDecoder =
    JD.succeed SignatoryLinkRecord
        ...
        |> JDP.required "authentication_method_to_view_archived" authenticationToViewMethodDecoder
