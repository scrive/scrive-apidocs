...
documentStatusDecoder =
  ...
  |> JD.andThen
      (\s -> case s of
        "preparation" -> JD.succeed Preparation
        "pending" -> JD.succeed Pending
        "closed" -> JD.succeed Closed
        "canceled" -> JD.succeed Canceled
        "timedout" -> JD.succeed Timedout
        "rejected" -> JD.succeed Rejected
        "document_error" -> JD.succeed DocumentError
        _ -> JD.fail <| "Cannot parse DocumentStatus: " ++ s
      )
