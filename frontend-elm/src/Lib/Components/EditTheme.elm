...
type Msg = SetActiveThemeMsg ThemeID
...
update embed msg read = case msg of
  SetActiveThemeMsg id -> setActiveTheme read id
...
viewEditColor embed colorIdentifier state =
  let color = getThemeColor state colorIdentifier
      ...
      readonly = state.themeBeingEdited.id <= 0
      closePopoverOverlay =
...
viewEditTheme params read state =
  let readonly = state.themeBeingEdited.id <= 0
