...
update {embed} msg state = case msg of
  ...
  AddFlashMessageMsg message ->
    let id = state.nextId
        ...
        cmd = Process.sleep 10000 |> Task.perform (\_ -> embed <| StartFadeoutMsg id)
    in (newState, cmd)

  -- | Set state to `TransitionToInvisible` and schedule removal in 250ms.
  StartFadeoutMsg id ->
    let newAlerts = List.map (\message -> if message.id == id
                              then { message | messageState = TransitionToInvisible }
                              else message) state.alerts
        cmd = Process.sleep 250 |> Task.perform (\_ -> embed <| RemoveFlashMessageMsg id)
    in ({ state | alerts = newAlerts }, cmd)
...
addFlashMessage params = update params << AddFlashMessageMsg

initialState : State
initialState = { alerts = [], nextId = 1 }

{- View -}

view : Params msg -> State -> Html msg
view {embed} state =
  div [ class "flash" ] <|  -- flash container
    flip List.map state.alerts <| \{id, message, messageState} ->
      div [ class "flash-content-wrapper"
          , class <| case message of
              FlashSuccess _ -> "success"
              FlashError _ -> "error"
          , style "display" <| case messageState of
              Visible -> "block"
              TransitionToInvisible -> "none" ] [
        div [ class "flash-content" ] [
          div [ class "flash-body" ] [
            text <| case message of
                  FlashSuccess txt -> txt
                  FlashError txt -> txt ],
