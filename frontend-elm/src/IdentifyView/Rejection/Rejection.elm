...
import Http exposing (Error, post, expectWhatever, jsonBody)
import Html.Attributes exposing (class, autocomplete, value, placeholder, style)
import Html.Events exposing (onClick, onInput)
import Html exposing (Html, div, input, text, a, b, p, textarea)
...
type State = EnterMessage {message : String} | Complete | AlreadyRejected

type Msg = RejectButtonClickedMsg
...
update params state msg = case msg of
  RejectButtonClickedMsg -> case state of
  ...
  RejectCallbackMsg res -> case res of
    Ok () -> (Complete, Cmd.none)
    Err err ->
      let flashMessage = "Failed to reject the Flow"
          errorFields =
  ...
  UpdateTextareaMsg message -> case state of
    EnterMessage _ -> ( EnterMessage {message = message}, Cmd.none )
    _ -> ( state, Cmd.none )
