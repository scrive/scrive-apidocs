...
import Html exposing (Html, a, b, div, input, text)
import Html.Attributes exposing (autocomplete, class, placeholder, type_, value)
import Html.Events exposing (keyCode, on, onClick, onInput)
import Http exposing (Error, expectWhatever, post)
        ...
        SMSPinIdentifyToViewCallbackMsg res ->
            case res of
                ...
                Err err ->
                    case err of
                        Http.BadStatus 400 ->
                            -- wrong PIN results in a 400 response
                            let
                                flashMessage =
...
viewContent params state rejectionLink =
    case state of
        ...
        EnterPin { pin } ->
            div [ class "identify-box-content" ]
                [ div [ class "identify-with-pin-box" ]
                                        ...
                                        |> JD.andThen
                                            (\code ->
                                                if code == enterKeyCode then
                                                    JD.succeed (params.embed ConfirmPinButtonClickedMsg)

                                                else
                                                    JD.fail "non-enter key ignored"
                                            )
                              in
                              on "keydown" parseEnterMsg
                            ]
                            []
                        ]
                    , div [ class "identify-box-button" ]
