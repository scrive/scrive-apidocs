...
type instance RuleResult CreateTestDBWithConfData
...
addOracles = do
  ...
  void . addOracle $ \(CreateTestDBWithConfData _) -> do
    ...
    connString <- if tc
      then askOracle (TeamCityBuildDBConnString ())
      else return defConnString
    dbName <- if tc then askOracle (TeamCityBuildDBName ()) else return defDBName
    lConf  <- if tc
      then askOracle (TeamCityBuildLambdaConf ())
      else error "AWS Lambda configuration for tests must be defined" :: m String
    s3Conf <- if tc
      then askOracle (TeamCityBuildS3Conf ())
      else error "AWS S3 configuration for tests must be defined" :: m String
    cronInvoiceConf <- if tc
...
oracleHelpRule = do
  ...
  "help-env" ~> do
    ...
    explainVar "NGINX_CONF_RULES_PATH_ALTERNATIVE"
               "Used for generating NGINX urls_list.txt file"
    showVarVal nginxconfrulespathalternative

    tcmi <- askOracle (TeamCityBuildCronMonthlyInvoice ())
    explainVar "TEST_CRON_MONTHLY_INVOICE"
               "Used for configuration of testing of monthly invoices"
    showVarVal tcmi
