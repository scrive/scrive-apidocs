  ...
  where
    ...
    requireVersion prog args ver pre =
      requireVersion' prog args ver pre
        `catch` (\(_ :: IOException) ->
                  hPutStrLn stderr
                            ("Required program '" <> prog <> "' couldn't be found.")
                    >> exitFailure
                )
    ...
    requireVersion' prog args ver pre = do
      ...
      ver' <-
        (evaluate . readVersion $ out)
          `catch` (\(_ :: SomeException) ->
                    hPutStrLn
                        stderr
                        ("Parse error: " <> prog <> " " <> show args <> ": " <> out)
                      >> exitFailure
                  )
      when (ver' < ver) $ do
        hPutStrLn
...
-- | Server build rules
serverBuildRules
  :: UseNewBuild -> OptimisationLevel -> EnableExecutableDynamic -> CabalFile -> Rules ()
serverBuildRules newBuild opt exeDynamic cabalFile = do
  ifNewBuild newBuild
             (serverNewBuildRules opt exeDynamic cabalFile)
             (serverOldBuildRules opt exeDynamic cabalFile)
  "_build" </> "db-docs" </> "kontra.html" %> buildDBDocs

getCabalConfigureFlags :: OptimisationLevel -> EnableExecutableDynamic -> Action [String]
...
serverNewBuildRules opt exeDynamic cabalFile buildDir = do
  ...
  "_build/cabal-haddock.tar.gz" %> \_ -> do
    ...
    command_ [] "cabal" ["v2-haddock", "all"]
    command_ [Shell]
             "tar"
             ["-czf", "_build/cabal-haddock.tar.gz", buildDir </> "noopt/doc"]

  "cabal-clean" ~> cmd "cabal v2-clean"

serverOldBuildRules
  :: OptimisationLevel -> EnableExecutableDynamic -> CabalFile -> Rules ()
serverOldBuildRules opt exeDynamic cabalFile = do
  ...
  "_build/cabal-update" %>>> do
    sourceRoot <- askOracle (SourceRoot ())
    need ["cabal.config", sourceRoot </> "kontrakcja.cabal"]
...
serverTestRules newBuild opt cabalFile createDBWithConf testPatterns = do
  "kontrakcja_test.conf" %> \_ -> case createDBWithConf of
    CreateTestDBWithNewConf -> do
      (dbName, initialConnString, lConf, s3Conf, cronMonthlyInvoiceConf) <- askOracle
  ...
  "run-server-tests" ~> do
      ...
      if tc
        then do
          let cmdopt = [Shell] <> langEnv
              flags =
                ["--plain", "--output-dir _build/kontrakcja-test-artefacts"] <> baseFlags
          (Exit c, Stdouterr out) <- command cmdopt testSuiteExe flags
          liftIO $ hUnitForTeamCity c out
        else cmd testSuiteExe baseFlags
    testCoverage <- askOracle (BuildTestCoverage ())
    when testCoverage $ do
      ...
      command_
        [Shell]
        ("zip -r _build/coverage-reports.zip " <> "coverage-reports kontrakcja-test.tix")
        []
    ...
    withDB CreateTestDBWithNewConf act = do
      ...
      where
        mkDB connString dbName = do
          unit $ cmd
            "psql"
            [connString <> " dbname=postgres", "-c", "CREATE DATABASE " <> dbName <> ";"]
          unit $ cmd
            "psql"
        ...
        rmDB connString dbName = callProcess
          "psql"
...
serverFormatLintRules sourceRoot newBuild threads opt cabalFile flags = do
  ...
  "test-hs-outdated-deps" ~> do
    cmd
      ...
      [ "outdated"
      , "--exit-code"
      , if useNewBuild newBuild then "--v2-freeze-file" else "--freeze-file"
      ]

  "hindent" ~> do
    forM_ srcSubdirs $ \subdir -> onEachHsFile subdir ["hindent", "-XNoPatternSynonyms"]

  "list-extensions" ~> do
    forM_ (allExtensions cabalFile) $ \ext -> putNormal . display $ ext
...
frontendBuildRules newBuild = do
  "_build/npm-install" %>>> do
    ...
    command [EchoStdout True, Cwd $ sourceRoot </> "frontend"] "npm" ["install"]

  "_build/grunt-build" %>>> do
    sourceRoot <- askOracle (SourceRoot ())
    need ["_build/npm-install", "localization"]
    alwaysRerun
    command ([EchoStdout True, Cwd $ sourceRoot </> "frontend"] <> langEnv)
            "grunt"
...
frontendTestRules newBuild = do
  "grunt-eslint" ~> do
    sourceRoot <- askOracle (SourceRoot ())
    need ["_build/npm-install"]
    cmd [Cwd $ sourceRoot </> "frontend"] "grunt" ["eslint", gruntNewBuildArg newBuild]

  "grunt-coverage" ~> do
    sourceRoot <- askOracle (SourceRoot ())
    need ["_build/grunt-build"]
    command_ [Cwd $ sourceRoot </> "frontend"] "grunt" ["test", gruntNewBuildArg newBuild]
...
distributionRules newBuild opt = do
  "urls.txt" %> \_ -> do
    ...
    unless tc $ do
      fail
        $  "ERROR: routinglist executable is only built "
        <> "with Shake when running from TeamCity"
    when (null nginxconfrulespath) $ do
      fail "ERROR: NGINX_CONF_RULES_PATH is empty"
    let routingListPath = componentTargetPath newBuild opt . mkExeName $ "routinglist"
    ...
    command_ [FileStdin nginxconfrulespath]
             routingListPath
             ["urls.txt", nginxconfdefaultrule]
    command_ [FileStdin nginxconfrulespathalternative']
             routingListPath
  ...
  where
    ...
    copyBinariesRules binaryNames = whenNewBuild newBuild $ \_ ->
...
runTransifexDiffScript :: UseNewBuild -> OptimisationLevel -> [ShakeFlag] -> Action ()
...
runTransifexPushScript :: UseNewBuild -> OptimisationLevel -> [ShakeFlag] -> Action ()
...
runTransifexMergeScript :: UseNewBuild -> OptimisationLevel -> [ShakeFlag] -> Action ()
...
runDetectOldLocalizationsScript newBuild opt = do
  let scriptPath =
        componentTargetPath newBuild opt . mkExeName $ "detect_old_localizations"
  need [scriptPath]
  cmd scriptPath

runDetectOldTemplatesScript :: UseNewBuild -> OptimisationLevel -> Action ()
runDetectOldTemplatesScript newBuild opt = do
  let scriptPath = componentTargetPath newBuild opt . mkExeName $ "detect_old_templates"
  need [scriptPath]
  cmd scriptPath
...
runLocalization newBuild opt = do
  let exePath = componentTargetPath newBuild opt . mkExeName $ "localization"
  need [exePath]
  cmd exePath
