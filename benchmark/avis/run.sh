...
if [ "$#" -lt 14 ]; then
    echo "usage: $0 AUTHORCOMPANYID TEMPLATEID NUMBEROFPAGES FILEPATH SERVER PORT PROTOCOL TOKEN_CREDENTIALS_IDENTIFIER CLIENT_CREDENTIALS_IDENTIFIER TOKEN_CREDENTIALS_SECRET CLIENT_CREDENTIALS_SECRET NUMBER_OF_CONCURRENT_USERS NUMBER_OF_LOOPS RAMPUP_PERIOD"
    echo ""
    ...
    echo "RAMPUP_PERIOD - number of seconds before new concurrent user joins"
    exit 1
fi

JVM_ARGS="-DCookieManager.save.cookies=true" jmeter -n -t avis.jmx "-Jauthorcompanyid=$1" "-Jtemplateid=$2" "-Jnumberofpages=$3" "-Jfilepath=$4" "-Jserver=$5" "-Jport=$6" "-Jprotocol=$7" "-Jtoken_credentials_identifier=$8" "-Jclient_credentials_identifier=$9" "-Jtoken_credentials_secret=${10}" "-Jclient_credentials_secret=${11}" "-Jnumber_of_concurrent_users=${12}" "-Jnumber_of_loops=${13}" "-Jrampup_period=${14}"
