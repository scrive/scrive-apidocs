...
var DocumentViewSignatoryView = Backbone.View.extend({
  initialize: function (args) {
  ...
  render: function() {
      ...
      var name      = $('<div class="name" />').text(signatory.nameOrEmailOrMobile()).css(model.textstyle());
      titleinfo.append(name);
      box.append(titleinfo);

      var inner   = $('<div class="inner spacing" />');

      var numspace = $('<div class="details" />').css(model.textstyle());
      var company   = $('<div class="company" />').text(signatory.company()).css(model.textstyle());
      ...
      if (signatory.email() != '') {
        var email   = $('<div class="email field" />').text(localization.email + ": " + signatory.email()).attr('title', signatory.email()).css(model.textstyle());
        numspace.append(email);
      }

      if (signatory.mobile() != '') {
        var mobile   = $('<div class="mobile field" />').text(signatory.mobile()).attr('title', signatory.mobile()).css(model.textstyle());
...
window.DocumentViewSignatory = function(args) {
          var model = new DocumentViewSignatoryModel(args);
          var view =  new DocumentViewSignatoryView({model : model, el : $("<div/>")});
