define(['Backbone', 'legacy_code'], function() {
  ...
  var AccountSetupView = Backbone.View.extend({
    ...
    clearValidationMessages : function() {
      $(".failed-validation",this.el).remove();
    },
    render: function () {
      ...
      model.addValidator(function() {
        return passwordInput.value().validate(new PasswordValidation({
                    callback: function(t, e, v) {
                      $("<div class='failed-validation validation-failed-msg' />").append(v.message()).appendTo(passwordInput.el().parent());
                    },
      ...
      model.addValidator(function() {
        return password2Input.value().validate(new PasswordEqValidation({
                  callback: function(t, e, v) {
                      $("<div class='failed-validation validation-failed-msg' />").append(v.message()).appendTo(password2Input.el().parent());
                  },
