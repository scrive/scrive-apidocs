			...
			setValue: function(year, month, day)  {
				...
				for (var j = !begin ? -7 : 0, a, num; j < (!begin ? 35 : 42); j++) { 
					...
					week.append(a);
                                ...
                                var moment_date = moment(date); // this points to the last day displayed in the calendar
                                moment_date.date(-14);  // make sure moment_date falls into current month (last day always is from the next month/extra row)
                                if (!moment(min).isBefore(moment_date, 'month')) {
                                  pm.addClass(css.disabled);
                                }

                                if (!moment_date.isBefore(moment(max), 'month')) {
